...
type Validations() =
    ...
    member this.``valid Guid returns Ok`` () =
        ...
        Assert.AreEqual(result, Common.okResult)
    ...
    member this.``invalid Guid returns Error`` () =
        ...
        Assert.AreEqual(result, Common.errorResult)
    ...
    member this.``Guid is not Guid Empty returns Ok`` () =
        let result = (Guid.isNotEmpty (Guid.Parse("6fddb3c1-24c2-4e2e-8f57-98d0838c0c3f")) "error").Result
        Assert.AreEqual(result, Common.okResult)
    ...
    member this.``valid SHA-256 hash returns Ok`` () =
        let result = (String.isEmptyOrValidSha256Hash "67A1790DCA55B8803AD024EE28F616A284DF5DD7B8BA5F68B4B252A5E925AF79" "error").Result
        Assert.AreEqual(result, Common.okResult)
    ...
    member this.``string is member of discriminated union returns Ok`` () =
        let result = (DiscriminatedUnion.isMemberOf<Types.ReferenceType, string> "Checkpoint" "error").Result
        Assert.AreEqual(result, Common.okResult)

    [<Test>]
    [<Parallelizable>]
    member this.``string is not member of discriminated union returns Error`` () =
        let result = (DiscriminatedUnion.isMemberOf<Types.ReferenceType, string> "Not a member" "error").Result
        Assert.AreEqual(result, Common.errorResult)
