...
selected Branch identity and terminalize it. Watch, Connect, and Doctor remain deferred. Migration, rollback, journals,
...
Reference finalization from persisted facts. Save-enabled construction of the same phase remains Issue #872. Doctor
...
- One primary invariant: Reference finalization follows verified local completion, cleans only exact marker evidence
...
- One source for each decision: SQLite selects the pending finalization; disk Branch configuration classifies previous,
  selected, third, or unreadable identity; the lease serializes the finalization effects.
