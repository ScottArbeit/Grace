...
| REQ-019 | Verified-root completion boundary | #923 | `WDU-LC-006` succeeds with ephemeral `bytesChanged`; `WDU-LC-007` retains marker evidence after a completion failure. |
...
`LocalCompletion(target, operation, bytesChanged)`; `bytesChanged` is ephemeral and adds no persisted shape.
...
Closed #870 and superseded #899 are not packet artifacts.
