...
- Grace is not in production and has no production data to import, migrate, preserve, classify, or grandfather. Do not
  weaken contracts, validators, generated clients, runtime behavior, or tests to preserve imaginary old data.

## Data Compatibility Posture

Grace is still pre-production. There is no production data to import, migrate, preserve, classify, or grandfather.
...
- If current runtime code has a deliberate API sentinel, treat it only as current API behavior, not as data migration or
  old-data compatibility.
- Review findings that cite legacy or migration data need triage before implementation. If the only risk is nonexistent
  production data, correct the premise through documentation, source behavior, tests, or review response rather than
