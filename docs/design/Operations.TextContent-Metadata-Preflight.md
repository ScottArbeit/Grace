...
| TC4: retained uncertain upload | `DescriptionCallerCancellationRetainsEvidenceForRetry` inspects the upload, cancels the caller's HTTP request while the existing gate still blocks append, then releases the gate and retries. The object and ETag remain unchanged and exactly one description event is accepted. The original request may commit after its caller loses the response. Existing production cleanup classification and ownership are unchanged. |
...
## Cancellation-test correction

[GitHub Validate run 34400102274](https://github.com/ScottArbeit/Grace/actions/runs/34400102274) at `ac7e9f919c90bdaa0c437a3c48fffd6c1ef425bd` disproved the first failure-injection assumption. Resetting the independent rendezvous socket did not cancel the caller's HTTP request: the handler returned success and appended. Prematurely closing that socket also caused the fixture's later StreamWriter disposal to throw. The other 319 hosted cases passed and 31 were skipped; this failed run is not a passing delivery gate.

The correction cancels the actual HTTP request after upload inspection, before normal gate release. It checks that the caller observes cancellation, the still-blocked actor has not yet appended, and the uploaded evidence remains unchanged. After release, either the original request or its exact retry may append. A successful retry must leave exactly one matching description event and the original object/ETag. No production code or actor failure hook changes.

The bundle's separate `CancellationProbe` contains a disposable Linux ASP.NET transport experiment, source and output. All five attempts observed caller cancellation before normal release and server completion afterward, with normal gate disposal. This tests HTTP and rendezvous ordering without Grace storage or actors. The repaired hosted test project built in Release with zero warnings and errors; Fantomas, Markdown lint and all 13 embedded-file hash checks passed. The corrected hosted case still requires current-head GitHub Validate; the transport experiment does not replace it.
