...
## Explicit file rename: accepted next slice

Readiness: **Plan-ready**, Product V1, for `grace library rename <path> <new-name>` in the configured Windows working copy. Scott accepted the [checkpoint recommendations](https://github.com/ScottArbeit/Grace/issues/1037#issuecomment-5591691687); the [decision record](https://github.com/ScottArbeit/Grace/issues/1037#issuecomment-5591944045) and [experiment](design/Libraries.Rename-Experiment.md) close the command and rejection/resume decisions. Issue #1073 owns implementation and production acceptance of this unchanged contract.
...
Issue #1039 delivered the three-table Windows client in PR #1070. Issue #1071 delivered populated-Library onboarding in PR #1072. Issue #1073 implements explicit same-parent file rename and the accepted local type/object redesign, requirements LIB-012 through LIB-015, as one mainline slice from `2122f2e68f7f22a091ed2f4eadb93273169de8aa`. It co-delivers the readiness evidence without adopting that branch as ancestry. The [validation record](design/Libraries.Rename-Validation.md) maps requirements to actual production tests.

The propagation map in the [type plan](design/Libraries.Type-Plan.md#explicit-file-rename-propagation) names local command, durable operation, application/capture, documentation and test changes. Existing shared DTOs, server persistence/actors, HTTP routes, SDK, OpenAPI and generated clients already support rename and remain unchanged. WDU, Save, Reference, DirectoryVersion and Attachment contracts remain unchanged. Production delivery requires focused Windows and hosted two-copy tests, independent R1, accepted repairs and bounded R2 when needed, Shape Review and current-revision GitHub Validate. Later checkpoint candidates remain separate work.
