...
| LIB-016 | Explicit pause commits under the root lease and survives restart; all admitted Library entry paths reread pause before effects. Only explicit resume clears it. | Next pause/resume slice under Epic #1037; Library command, synchronization, repository-state persistence and Watch gates. | Pause/resume transaction interruption and canceled wait; stale cached status and competing resume/pause; real running Watch plus CLI positive/negative controls. |
...
Issue #1039 delivered the three-table Windows client in PR #1070. Issue #1071 delivered populated-Library onboarding in PR #1072. Issue #1073 delivered explicit same-parent file rename and the accepted local type/object redesign, requirements LIB-012 through LIB-015, in PR #1074. Its [validation record](design/Libraries.Rename-Validation.md) maps requirements to actual production tests. The next accepted pause/resume slice covers LIB-016 through LIB-018; its implementation child is not yet created.

The [type plan](design/Libraries.Type-Plan.md) records the delivered rename propagation and the planned pause/resume propagation. PR #1074 completed focused Windows/hosted tests, R1, Shape Review and Validate before its exact approved landing. Equivalent gates remain required for the future pause/resume implementation. Remote DTOs, server ownership, HTTP/SDK/generated clients and WDU/Save/Reference/DirectoryVersion/Attachment contracts remain unchanged by the selected local pause behavior. Later checkpoint candidates remain separate work.
