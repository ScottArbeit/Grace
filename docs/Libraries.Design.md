...
The next selected outcome lets a user explicitly rename one synchronized nonempty file within its directory while both Windows copies retain the same item and content history. The accepted command and failure behavior appear below. Automatic recognition of an Explorer rename remains deferred.
...
As of 2026-09-08, PR #1043 and PR #1044 are closed unmerged as superseded; the historical evidence table does not describe active candidates. Their branches/worktrees and the accepted-design revision `e57998c4c56c22fffaa47d206831be7579ecb76b` remain preserved. The current readiness base is `8ce7cd4dda7089e801662dfc410bab4dcbf0ff81`.
...
| LIB-012 | Explicit same-parent rename preserves one item and content revision; exact intent survives retry and accepted application follows genuine feed predecessors. | Next selected child; Library.CLI, LibrarySynchronization and LibraryLocalState. | Real command in A, actual accepted rename and ordered compatible edits, exact bytes/new path in A/B, interruption/replay and completed restart without create/upload/rewrite. |
| LIB-013 | Only a definitively rejected unprepared rename becomes terminal without item/cursor/filename effects; unrelated sync can continue. | Next selected child; narrow local operation receipt/retirement and CLI outcome. | Rejection response loss, transaction rollback, competing rename/name/delete, unrelated catch-up, existing pruning eligibility and saved-content rejection remains pending. |
| LIB-014 | Rename effects preserve changed/zero-byte input and exact Watch classification across publication/removal/completion. | Next selected child; existing Windows application, local completion and capture seams. | Source/destination edits and obstructions, catalog changes, real lease contention, exact prepared suppression plus changed-file and untracked-file positive capture, failed completion guards and restart timestamps. |
...
Issue #1039 delivered the three-table Windows client in PR #1070. Issue #1071 delivered populated-Library onboarding in PR #1072. The next selected slice is explicit same-parent file rename, requirements LIB-012 through LIB-014, with no prerequisite production PR identified. Start a fresh implementation session, refresh `origin/main`, and establish one native child issue, its readiness gate and a pinned mainline-slice run charter. Use controller/worker mode with one implementation owner; the readiness branch is evidence to co-deliver, not required ancestry or an epic integration branch.
