...
Issue #1039 delivered the three-table Windows client in PR #1070. The following checkpoint selected only Issue #1071 populated-Library onboarding. It changes local records, CLI enable/run/status behavior, tests and maintained documentation. Existing shared DTOs, server persistence, HTTP routes, SDK, OpenAPI and generated clients provide the required metadata and retained revisions and remain unchanged. WDU, Save, Reference, DirectoryVersion and Attachment contracts remain unchanged. Later checkpoint candidates are separate work.
