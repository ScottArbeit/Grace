...
<!-- markdownlint-disable-next-line MD033 -->
<a id="additive-catalog-adoption"></a>
...
When an administrator adds Library2, every copy with Library synchronization enabled discovers it through notification or on restart/reconnect, creates its missing local directory and downloads its contents automatically. An empty Library must also become visible locally. Synchronize every Library in the repository as quickly as possible; selective synchronization remains deferred. Several additions while a copy is away are ordinary input, not a reason to require a manual command.

No `adopt-catalog` command, manual pause, clean-tree preparation, manual directory creation or Watch restart is part of receiving a Library. A copy already explicitly paused remains paused until the delivered resume action. A running active copy updates its Library classification and begins catch-up without ending Watch. Startup discovers changes missed while it was stopped. Notifications are hints; authenticated catalog and change reads determine the work, with periodic recovery from missed hints.

Preserve working-copy identity, existing materialized bases, typed operations, verified saved objects, terminal/echo history, GUID epoch and the opaque signed applied cursor. Selecting a catalog does not mean its content has been applied and cannot move the cursor past unfinished changes. Existing local edits and pending work must remain recoverable; receiving a Library cannot require users to clean their other Libraries first. A path obstruction remains protected and is reported specifically. Do not replace unrelated files or infer that an arbitrary occupied directory belongs to Grace.
...
The [selected approach](https://github.com/ScottArbeit/Grace/issues/1081#issuecomment-5616262920), under Scott's authorization to make implementation decisions, prevents an unrelated root addition from invalidating an ordinary queued save. Add `AdditiveCatalogVersions` to the existing `LibraryControlDocument`, using a new serializer field identity without renumbering existing fields. Each committed addition retains its known predecessor as compatible; a removal clears predecessor compatibility. Strict additions and the 128-root limit bound this retained set. Commit it with the catalog and cleared Pending in the existing protocol. This is an extension to that record, not a new lifecycle or store; its actor experiment is still required.

Item submission accepts the current catalog version or a known additive predecessor after the existing current authorization, path ownership, namespace/content and upload checks. Unknown versions remain stale. Catalog administration still requires its exact current version. Preserve each original operation ID, request and hash. Existing recorded receipts are returned unchanged before new acceptance checks; no rejected request is revived, replaced or migrated. This prevents a new unrelated-addition rejection rather than inventing automatic reissue of rejected saved work.

Use the existing `LibraryContentAvailable.v1` advisory payload with the current content cursor and new catalog version for a best-effort catalog wake, with message identity derived from the catalog version. Keep content notification progress and its failed-envelope slot unchanged. Startup and periodic reads recover missed hints. Initial enable must also handle all configured roots; do not retain the single-root guard as a product limitation.

Before replacement production work, run a disposable Windows/SQLite experiment that determines the effect order for catalog selection, missing-directory creation and ordinary retained-history application. Exercise real existing local-state and incoming-operation seams wherever their guards determine feasibility. Test several missed additive versions and paged retained history, existing saved or pending work, partial incoming application, pause, and interruptions around directory creation, selection, byte publication and SQLite completion. Discard memory and reopen durable state after each injected interruption. Do not treat the old clean paused experiment as evidence for these cases.
...
| LIB-019 | Enabled active copies automatically discover every added Library through a wake or startup/reconnect and create missing roots, without a manual adoption command or Watch restart. | Issue #1081 revised design; actor notification, Watch policy and finite synchronization. | Actual empty-root addition during live Watch; missed notifications and several additions while stopped; automatic local creation and content download. |
| LIB-020 | Catalog changes preserve saved objects, frozen requests, materialized history and explicit pause; obstructions are retained and reported. | Issue #1081 revised design; root lease, local-state and filesystem guards. | Pending and rejected work, local edits, partial incoming application, occupied/reparse paths, stale reads, canceled lease and paused negative control. |
| LIB-021 | New-root content and old-root backlog apply in genuine feed order from retained progress, including paging and interruption/restart. | Issue #1081 revised design; incoming selection, preparation and completion. | Paged old/new catalog history; multiple missed additions; failure around root creation, catalog commit and byte publication; new-process recovery without duplicate publication or completed-file rewrite. |
...
Issue #1081 is being replanned for corrected LIB-019 through LIB-021. The old manual [experiment](design/Libraries.Catalog-Adoption-Experiment.md) and candidate results remain historical; the [validation mapping](design/Libraries.Catalog-Adoption-Validation.md) lists automatic behavior still to test. A replacement algorithm experiment and charter precede runtime work. Independent review, current-head GitHub Validate and exact-head merge approval apply to the replacement candidate.
