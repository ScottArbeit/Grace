...
<!-- Retain the link target used by the captured readiness report. -->
<!-- markdownlint-disable-next-line MD033 -->
<a id="additive-catalog-adoption-accepted-not-implemented"></a>

## Additive catalog adoption

**Implementation: [Issue #1081](https://github.com/ScottArbeit/Grace/issues/1081), with independent review and CI pending.** The [accepted D1-D3 decisions](https://github.com/ScottArbeit/Grace/issues/1037#issuecomment-5610931795) and [bounded experiment](design/Libraries.Catalog-Adoption-Experiment.md) preceded this mainline slice from `9b18c97ab342d319d3bdeaedc5f1d64571913cf5`. The [production validation record](design/Libraries.Catalog-Adoption-Validation.md) separates actual command/SQLite/hosted results from the earlier prototype.
...
`grace library sync adopt-catalog` uses the existing `LibrarySynchronizationStatus` output shape. The command selects the current server catalog; it does not accept an arbitrary replacement catalog or create a server catalog change. Catalog administration remains a separate existing operation. Existing repository-scoped read permissions suffice to adopt an already configured root locally. The complete catalog retains the server's root order; each root is normalized and non-overlapping.
...
| LIB-019 | Explicit adoption of one direct additive catalog successor commits only the local catalog under the root lease, preserving pause, identity, cursor, epoch and all existing durable work. | Issue #1081 implementation, Library.CLI, LibrarySynchronization and LibraryLocalState. | Actual command result; before/inside/after commit interruption, reopened state and duplicate invocation; exact row/item/operation/object retention. |
| LIB-020 | Adoption refuses unsupported participation, pending work, disk input, root transitions and unavailable history; stale observations cannot produce Library effects while paused. | Issue #1081 implementation, repository/materialization reads, filesystem checks, root lease and Watch boundary. | Actual CLI exclusions and canceled wait; production clean-tree positive/negative controls; partial page/frozen/rejected/prepared work; catalog/epoch/history and physical-path stops; queued Watch classification with exact retention. |
| LIB-021 | Explicit resume after adoption applies retained old-root backlog and added-root content in genuine feed order without rewriting accepted changes or frozen requests. | Issue #1081 implementation, incoming operation preparation and completion, existing finite synchronization. | Actual two-copy CLI/HTTP tracer; unsupported history rejected before effects; local-request completion restriction retained; publication-before-completion interruption and new-process restart without duplicate upload or completed-file rewrite. |
...
Issue #1081 implements LIB-019 through LIB-021. The [experiment](design/Libraries.Catalog-Adoption-Experiment.md) remains historical; the [production validation record](design/Libraries.Catalog-Adoption-Validation.md) maps the actual command, local transaction, refusal tests and hosted restart. Independent review, final-head GitHub Validate and exact-head merge approval remain delivery gates.
