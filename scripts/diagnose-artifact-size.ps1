...
function Test-ArtifactSizeResponse {
...
function Invoke-ArtifactSizeDiagnosis {
    ...
    Write-Host "Artifact declaration diagnostic saved to '$destination'."
}

if ($MyInvocation.InvocationName -ne '.') {
    try {
        Invoke-ArtifactSizeDiagnosis -BoundParameters $PSBoundParameters
        exit 0
    }
    catch {
