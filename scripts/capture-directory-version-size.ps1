...
function Test-DirectoryVersionObservationResponse {
...
function Invoke-DirectoryVersionObservation {
    ...
    $route = "/admin/directory-version-size/observations/$observationId"
    ...
    Write-Host "DirectoryVersion declaration observation saved to '$destination'."
}

if ($MyInvocation.InvocationName -ne '.') {
    try {
        Invoke-DirectoryVersionObservation -BoundParameters $PSBoundParameters
        exit 0
    }
    catch {
