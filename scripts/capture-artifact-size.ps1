...
function Test-ArtifactObservationResponse {
    ...
    try {
        ...
        foreach ($name in @('DeclaredArtifactBytes', 'DistinctArtifactCount')) {
            $quantity = 0L
...
function Invoke-ArtifactObservation {
    ...
    $route = "/admin/artifact-size/observations/$observationId"
    $request = @{ Uri = [uri]::new($serverUri, $route); Method = 'Post';
        Headers = @{ Authorization = "Bearer $($env:GRACE_TOKEN)" }; ContentType = 'application/json';
        Body = ($scope | ConvertTo-Json -Compress); SkipHttpErrorCheck = $true; ConnectionTimeoutSeconds = 0; OperationTimeoutSeconds = 0 }
    if ($mode -eq 'Read') {
    ...
    try { Test-ArtifactObservationResponse -Json $response.Content -Scope $scope -ObservationId $observationId }
    ...
    catch { throw "Local output publication failed. Prior output was not changed. $retryAdvice" }
    finally { if ([IO.File]::Exists($temporary)) { Remove-Item -LiteralPath $temporary -Force } }
    Write-Host "Artifact declaration observation saved to '$destination'."
}

if ($MyInvocation.InvocationName -ne '.') {
    try {
        Invoke-ArtifactObservation -BoundParameters $PSBoundParameters
        exit 0
    }
    catch {
