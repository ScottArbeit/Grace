...
function Test-TextContentSizeResponse {
...
function Invoke-TextContentSizeDiagnosis {
    ...
    Write-Host "TextContent declaration diagnostic saved to '$destination'."
}

if ($MyInvocation.InvocationName -ne '.') {
    try {
        Invoke-TextContentSizeDiagnosis -BoundParameters $PSBoundParameters
        exit 0
    }
    catch {
