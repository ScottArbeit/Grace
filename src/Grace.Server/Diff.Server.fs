...
module Diff =
    ...
    /// Retrieves a diff taken by comparing two DirectoryVersions by Sha256Hash or unique Sha256Hash prefix.
                ...
                try
                    let validations (parameters: GetDiffBySha256HashParameters) =
                        [|
                            ...
                            String.isValidSha256HashPrefix parameters.Sha256Hash1 DiffError.InvalidSha256Hash
                    ...
                    if validationsPassed then
                        ...
                        let! directoryVersionResolution1 = getDirectoryVersionResolutionBySha256Hash repositoryId parameters.Sha256Hash1 correlationId

                        let! directoryVersionResolution2 = getDirectoryVersionResolutionBySha256Hash repositoryId parameters.Sha256Hash2 correlationId
                        ...
                        | NoMatches, _ ->
                            return!
                                context
                                |> result400BadRequest (
                                    diffHashError context $"No DirectoryVersion matched Sha256Hash1 prefix '{parameters.Sha256Hash1}' in repository scope."
                                )
                        | _, NoMatches ->
                            return!
                                context
                                |> result400BadRequest (
                                    diffHashError context $"No DirectoryVersion matched Sha256Hash2 prefix '{parameters.Sha256Hash2}' in repository scope."
                                )
                        | AmbiguousMatches _, _ ->
                            return!
                                context
                                |> result400BadRequest (
                                    diffHashError context $"Sha256Hash1 prefix '{parameters.Sha256Hash1}' is ambiguous in repository scope."
                                )
                        | _, AmbiguousMatches _ ->
                            return!
                                context
                                |> result400BadRequest (
                                    diffHashError context $"Sha256Hash2 prefix '{parameters.Sha256Hash2}' is ambiguous in repository scope."
                                )
                    else
                        let! error = validationResults |> getFirstError
