...
module LibraryCoordinator =
    ...
    type LibraryCursorCodec(secret: byte array) =
        ...
        do
            if isNull secret || secret.Length < 32 then
                invalidArg (nameof secret) "The Library cursor secret must contain at least 32 bytes."
        ...
        interface ILibraryCursorCodec with
    ...
    let deterministicGuid (repositoryId: RepositoryId) (operationId: LibraryOperationId) label =
        let bytes =
    ...
    /// Returns the public item state persisted after one canonical change.
    let private resultItem (pending: LibraryPendingCommandDocument) =
    ...
    let private occupiedSlot repositoryId cursor (item: LibraryItemDto) =
        let namespaceValue =
            item.Namespace
            |> Option.defaultWith (fun () -> invalidOp "A live Library item must contain namespace state.")

        {
            id = $"slot:{LibraryPersistence.pathHash namespaceValue.NormalizedPath}"
    ...
    let private vacantSlot repositoryId cursor operationId (namespaceValue: LibraryNamespaceDto) =
        {
            id = $"slot:{LibraryPersistence.pathHash namespaceValue.NormalizedPath}"
            RepositoryId = repositoryId
            ProjectionKind = "slot"
    ...
    let private resolveParentPath (store: ILibraryStore) repositoryId parent cancellationToken =
        task {
    ...
    /// Reports whether one normalized namespace path belongs to the exact current Library set.
    let private isOwnedByRoot (libraryCatalog: LibraryCatalogDto) normalizedPath =
    ...
    let private rejectedReceipt
        ...
        =
        {
            ...
            Conflict = None
            ReasonCode = reason
            CurrentLibraryCatalog = if outcome = OutcomeKind.StalePolicy then Some libraryCatalog else None
            Rebaseline = None
        }

    /// Persists one deterministic non-canonical receipt without advancing the accepted change stream.
        ...
        store.UpsertReceiptAsync(
            {
                ...
                RepositoryId = repositoryId
                RecordKind = "receipt"
                RecordKey = $"operation:{receipt.OperationId:D}"
                SchemaVersion = 1
    ...
    let private decide
        ...
        task {
            ...
            if command.LibraryCatalogVersion
               <> control.LibraryCatalog.Version then
                return Error(rejectedReceipt command principalId now OutcomeKind.StalePolicy None control.LibraryCatalog None)
            else
                let! currentItem =
                ...
                let! destination =
                    task {
                        match command.ChangeKind, command.CreationSlotExpectation, command.DestinationParent, command.DestinationName, currentItem with
                        | ChangeKind.CreateFile, Some slot, _, _, _
                        | ChangeKind.CreateDirectory, Some slot, _, _, _ ->
                            return! resolvePath store command.RepositoryId slot.Parent slot.Name cancellationToken
                        | ChangeKind.Rename, _, _, Some name, Some item ->
                            ...
                            | None -> return None
                        | ChangeKind.Move, _, Some parent, _, Some item ->
                            match item.Item.Namespace with
                ...
                let currentPathIsOwned =
                    currentNamespace
                    |> Option.forall (fun namespaceValue -> isOwnedByRoot control.LibraryCatalog namespaceValue.NormalizedPath)

                let destinationPathIsOwned =
                    destination
                    |> Option.forall (isOwnedByRoot control.LibraryCatalog)
                ...
                let! directoryNotEmpty =
                    match command.ChangeKind, currentItem with
                    | ChangeKind.Delete, Some document when document.Item.ItemKind = ItemKind.Directory ->
                ...
                let itemMissingReceipt reason = rejectedReceipt command principalId now OutcomeKind.Rejected (Some reason) control.LibraryCatalog None
                ...
                if command.ItemId.IsSome && currentItem.IsNone then
                    ...
                    if not namespaceMatches then
                        return Error(itemMissingReceipt RejectionReason.NamespaceChanged)
                         ...
                         && command.ChangeKind <> ChangeKind.UpdateContent then
                        ...
                        | ChangeKind.CreateDirectory ->
                            ...
                            contentValue <-
                                if command.ChangeKind = ChangeKind.CreateFile then
                                    ...
                                    None
                        | ChangeKind.UpdateContent when not contentMatches ->
                        ...
                        | ChangeKind.UpdateContent -> contentValue <- command.PreparedContent
                        | ChangeKind.Rename ->
                        ...
                        let streamSegment = LibraryPersistence.segmentKey (LibraryPersistence.changeSegment cursor)

                        let canonical =
                            {
                                id = $"cursor:{LibraryPersistence.segmentKey cursor}"
        ...
        interface ILibraryCoordinator with
            ...
            member _.SubmitAsync(command, principalId, correlationId, cancellationToken) =
                task {
                    ...
                    | None ->
                        let mutable finished = false
                        let mutable result = Unchecked.defaultof<LibraryOperationReceiptDto>
                        ...
                        while not finished do
                            ...
                            if control.Document.Pending.IsSome then
                                ...
                                | Ok pending ->
                                    ...
                                    match pending.Receipt.Item, capacityPath with
                                    | Some item, Some normalizedPath ->
                                        do! store.EnsureCurrentProjectionCapacityAsync(command.RepositoryId, item.ItemId, normalizedPath, cancellationToken)
                                    | _ -> invalidOp "An accepted Library change must identify its current or prior namespace path."
                    ...
                    return
                        {
                            ...
                            RepositoryId = repositoryId
                            LibraryCatalogVersion = control.Document.LibraryCatalog.Version
