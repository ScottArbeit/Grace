        ...
        fun (next: HttpFunc) (context: HttpContext) ->
            task {
                ...
                try
                    ...
                    log.LogInformation("{CurrentInstant}: Finished {path}; RepositoryId: {repositoryId}; CorrelationId: {correlationId}; Duration: {duration}ms.", getCurrentInstantExtended(), context.Request.Path, graceIds.RepositoryId, (getCorrelationId context), duration_ms)
                    return result
                with ex ->
                    let duration_ms = (getCurrentInstant().Minus(startTime).TotalMilliseconds).ToString("F3")
                    log.LogError(ex, "{CurrentInstant}: Error in {path}; RepositoryId: {repositoryId}; CorrelationId: {correlationId}; Duration: {duration}ms.", getCurrentInstantExtended(), context.Request.Path, graceIds.RepositoryId, (getCorrelationId context), duration_ms)
                    return! context |> result500ServerError (GraceError.Create $"{createExceptionResponse ex}" (getCorrelationId context))
        }
