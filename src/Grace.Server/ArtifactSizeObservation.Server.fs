...
module ArtifactSizeObservation =
    /// Reads prior SQL state first, then checks current repository scope on both sides of a new collection.
    let internal captureWith
        (lookup: CancellationToken -> Task<Result<ArtifactSizeObservation option, string>>)
        (checkRepository: CancellationToken -> Task<unit>)
        (collect: CancellationToken -> Task<ArtifactSizeObservation>)
        (accept: ArtifactSizeObservation -> CancellationToken -> Task<Result<ArtifactSizeObservation, string>>)
        ...
        fun next context ->
            task {
                ...
                try
                    ...
                    match Artifact.validateArtifactSizeDiagnosticParameters parameters with
                    ...
                    | Ok scope ->
                        ...
                        let lookup = ArtifactSizeObservations.lookup connectionString observationId scope
                                ...
                                task {
                                    let! accepted =
                                        captureWith
                                            ...
                                            (fun token ->
                                                task {
                                                    let! bytes, count, started, finished = readArtifactSize scope token

                                                    return
                                                        {
                                                            ObservationId = observationId
                                                            Scope = scope
                                                            DeclaredArtifactBytes = bytes
                                                            DistinctArtifactCount = count
                                                            EnumerationStartedAt = started
                                                            EnumerationFinishedAt = finished
                                                        }
                                                })
                                            (ArtifactSizeObservations.accept connectionString)
