...
/// Holds one managed artifact root coupled to the already-owned private CacheStore database.
...
module CacheArtifactStore =
    ...
    /// Creates the one reset table that holds only exact Staging and Complete artifact tuples.
    ...
    /// Opens one managed artifact root after the private cache database has already gained process ownership.
                ...
                CacheStore.withBusyRetry (fun () ->
                    ...
                    | Absent ->
                        ...
                        executeEffect failurePoint StagingFileCreation (fun () ->
                            use _stream = new FileStream(stagedPath, FileMode.CreateNew, FileAccess.Write, FileShare.None) in ())
