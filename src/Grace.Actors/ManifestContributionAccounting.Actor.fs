...
type IExactRelationshipStore =
    ...
    /// Ensures one current relationship exists using ExactRelationshipKey.create rather than an adapter-local key format.
    ...
    /// Ensures one current relationship is absent using the same ExactRelationshipKey.create identity as presence writes.
    ...
    /// Enumerates a partition encoded only through ExactRelationshipKey.createPartitionKey so writes and reads cannot diverge.
    ...
    /// Re-reads one ExactRelationshipKey.create identity directly so callers never infer absence from a cache or timeout.
