...
module RepositoryContentCounter =
    ...
    let completeTrackedAddForKey expectedPrimaryKey (counter: RepositoryContentCounterDto) operationId (metadata: EventMetadata) =
        if String.IsNullOrWhiteSpace operationId then
            Error(graceError metadata.CorrelationId "RepositoryContentCounter tracked add completion requires an operation id.")
    ...
    /// Persists one Library add together with the bounded identity needed to replay its workflow handoff.
