...
module Services =
    ...
    let private azureStorageConnectionString =
        Environment.GetEnvironmentVariable(Constants.EnvironmentVariables.AzureStorageConnectionString)
    ...
    let private sharedKeyCredential =
        StorageSharedKeyCredential(DefaultObjectStorageAccount, storageKey)
    ...
    let getOrganizationNameActorId ownerId organizationName =
        ActorId($"{organizationName}|{ownerId}")

    let getRepositoryNameActorId ownerId organizationId repositoryName =
    ...
    let getAzureBlobClient (repositoryDto: RepositoryDto) (fileVersion: FileVersion) (correlationId: CorrelationId) =
        task {
            ...
            let containerNameActorProxy =
                actorProxyFactory.CreateActorProxy<IContainerNameActor>(containerNameActorId, ActorName.ContainerName)
            ...
            | Ok containerName ->
                ...
                let blobClient =
                    containerClient.GetBlobClient($"{fileVersion.RelativePath}/{fileVersion.GetObjectFileName}")
    ...
    let resolveOwnerId (ownerId: string) (ownerName: string) (correlationId: CorrelationId) =
        task {
            ...
            if not <| String.IsNullOrEmpty(ownerId) && Guid.TryParse(ownerId, &ownerGuid) then
                ...
                let cached = memoryCache.TryGetValue(ownerGuid, &exists)
                ...
                if cached && not <| exists then
                    ...
                    let actorProxy =
                        actorProxyFactory.CreateActorProxy<IOwnerActor>(ActorId(ownerId), ActorName.Owner)
                ...
                if ownerGuid.Equals(Constants.MemoryCacheValueGuid) then
                    ...
                    | None ->
                        ...
                        | AzureCosmosDb ->
                            let queryDefinition =
                                QueryDefinition(
                                    """SELECT c["value"].OwnerId FROM c WHERE STRINGEQUALS(c["value"].OwnerName, @ownerName, true) AND c["value"].Class = @class"""
                                )
    ...
    let resolveOrganizationId
        ...
        task {
            ...
            | Some ownerId ->
                ...
                then
                    let mutable exists = false
                    let cached = memoryCache.TryGetValue(organizationGuid, &exists)
                        ...
                        if repositoryGuid.Equals(Constants.MemoryCacheValueGuid) then
                            ...
                            | None ->
                                ...
                                | AzureCosmosDb ->
                                    ...
                                    let iterator =
                                        cosmosContainer.GetItemQueryIterator<RepositoryIdRecord>(queryDefinition)
        ...
        task {
            ...
            if not <| String.IsNullOrEmpty(branchId) then
                ...
                if cached && not <| exists then
                    ...
                    let actorProxy =
                        actorProxyFactory.CreateActorProxy<IBranchActor>(ActorId(branchId), ActorName.Branch)
                ...
                if branchGuid.Equals(Constants.MemoryCacheValueGuid) then
                    ...
                    | None ->
                        ...
                        | AzureCosmosDb ->
                            let queryDefinition =
                                QueryDefinition(
                                    """SELECT c["value"].BranchId FROM c WHERE STRINGEQUALS(c["value"].BranchName, @branchName, true) AND c["value"].RepositoryId = @repositoryId AND c["value"].Class = @class"""
                                )
                            ...
                            if iterator.HasMoreResults then
                                ...
                                if String.IsNullOrEmpty(branchId) then
                                    // We didn't find the BranchId, so add this BranchName to the MemoryCache and indicate that we have already checked.
                                    use newCacheEntry =
                                        memoryCache.CreateEntry(
                                            $"BrN:{branchName}",
                                            Value = Constants.MemoryCacheValueGuid,
    ...
    type DirectoryVersionEventValue() =
        member val public event: DirectoryVersion.DirectoryVersionEvent =
            { Event = DirectoryVersion.DirectoryVersionEventType.PhysicalDeleted
              Metadata = (EventMetadata.New String.Empty String.Empty) } with get, set
        ...
        task {
            ...
            | AzureCosmosDb ->
                try
                    ...
                    let queryDefinition =
                        QueryDefinition(
                            $"""SELECT TOP @maxCount c["value"] FROM c WHERE c["value"].RepositoryId = @repositoryId AND c["value"].Class = @class {includeDeletedClause} ORDER BY c["value"].CreatedAt DESC"""
                        )
                            .WithParameter("@repositoryId", repositoryId)
                            .WithParameter("@maxCount", maxCount)
                            .WithParameter("@class", "BranchDto")
    ...
    let getReferences (branchId: BranchId) (maxCount: int) =
        task {
            ...
            | AzureCosmosDb ->
                ...
                let queryDefinition =
                    QueryDefinition(
                        """SELECT TOP @maxCount c["value"].Class, c["value"].ReferenceId, c["value"].BranchId, c["value"].DirectoryId, c["value"].Sha256Hash, c["value"].ReferenceType, c["value"].ReferenceText, c["value"].CreatedAt FROM c WHERE c["value"].BranchId = @branchId AND c["value"].Class = @class ORDER BY c["value"].CreatedAt DESC"""
                    )
        ...
        task {
            ...
            try
                ...
                while iterator.HasMoreResults do
                    ...
                    let rps =
                        float (results.Resource.Count())
                        / (getCurrentInstant().Minus(startTime).TotalSeconds)
    ...
    let getReferencesLinq branchId =
        task {
            ...
            | AzureCosmosDb ->
                ...
                let query =
                    cosmosContainer
                        .GetItemLinqQueryable<ReferenceDto>(requestOptions = queryRequestOptions)
    ...
    let getLatestReference branchId =
        task {
            ...
            | AzureCosmosDb ->
                ...
                let queryDefinition =
                    QueryDefinition(
                        """SELECT TOP 1 c["value"] FROM c WHERE c["value"].BranchId = @branchId AND c["value"].Class = @class ORDER BY c["value"].CreatedAt DESC"""
                    )
                        .WithParameter("@branchId", $"{branchId}")
            ...
            | AzureCosmosDb ->
                ...
                while directoryIdQueue.Count > 0 && allExist do
                    ...
                    while iterator.HasMoreResults do
                        ...
                        if not <| results.Resource.Any() then
                            allExist <- false
        ...
        task {
            ...
            | AzureCosmosDb ->
                ...
                while iterator.HasMoreResults do
                    ...
                    results.Resource
                    |> Seq.iter (fun refDto -> queryResults.Add(refDto.value.ReferenceId, refDto.value))
