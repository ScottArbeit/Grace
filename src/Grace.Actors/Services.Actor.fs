...
module Services =
    ...
    let private directoryVersionDeclarations
        (validateManifest: CorrelationId -> FileVersion -> FileManifest -> Result<unit, GraceError>)
        (scope: UsageFactScope)
        correlationId
        (row: DirectoryVersionEventValue)
        =
        if isNull (box row)
           || isNull row.State
        ...
        let created =
            ...
            |> Array.choose (fun event ->
                ...
                | DirectoryVersionEventType.Created directory -> Some directory
                | _ -> None)

        if created.Length <> 1
           || (match row.State[0].Event with
               | DirectoryVersionEventType.Created _ -> false
               | _ -> true) then
        ...
        |> Seq.map (fun file ->
            ...
            | FileContentReferenceType.FileManifest, Some manifest ->
                match validateManifest correlationId file manifest with
    ...
    /// Counts distinct declarations to exhaustion, using the existing manifest validator and caller cancellation.
    let internal enumerateDirectoryVersionSizeWith
        validateManifest
        ...
        task {
            ...
            while more do
                ...
                while index < rows.Length do
                    ...
                    use entries =
                        (directoryVersionDeclarations validateManifest scope correlationId rows[index])
                            .GetEnumerator()
            ...
            return total, int64 values.Count, started, getCurrentInstant ()
        }

    /// Requires persisted declaration fields while honoring the serializer's omitted Size encoding for zero-length files.
    let internal decodeDirectoryVersionSizeDocument (document: JsonElement) =
    ...
    let internal readDirectoryVersionSizeFromContainer (container: Container) validateManifest scope correlationId cancellationToken =
        task {
            let query =
                ...
                enumerateDirectoryVersionSizeWith
                    ...
                    (fun token ->
                        task {
                            ...
                            return
                                page.Resource
                                |> Seq.map decodeDirectoryVersionSizeDocument
                                |> Seq.toArray,
    ...
    /// Selects the configured actor storage provider before acquiring its container for a declaration scan.
    let internal readDirectoryVersionSizeWith provider getContainer validateManifest scope correlationId (cancellationToken: CancellationToken) =
        task {
            match provider with
            | AzureCosmosDb ->
                cancellationToken.ThrowIfCancellationRequested()
                return! readDirectoryVersionSizeFromContainer (getContainer ()) validateManifest scope correlationId cancellationToken
            | MongoDB
            | Unknown -> return raise (NotSupportedException "DirectoryVersion size diagnosis is not supported by the configured actor state storage provider.")
        }

    /// Reads declarations through Grace's current provider selection and the caller's existing manifest validation.
