...
[<assembly: InternalsVisibleTo("Grace.Cache.Storage.Tests")>]
do ()

/// Reports the SQLite settings verified whenever a storage store gains process ownership.
type CacheStoreDiagnostics = { SchemaVersion: int; JournalMode: string; ForeignKeysEnabled: bool; BusyTimeoutMilliseconds: int }

/// Holds one process-owned database lock and shared lifetime state for store leases.
...
/// Represents a private lease over the one process-owned storage database.
type CacheStore = private { Shared: CacheStoreShared; mutable Released: int }

/// Returns an opened private store or the stable result when another process owns its database.
type CacheStoreOpenResult =
    | Opened of store: CacheStore
    | CacheDatabaseInUse
...
/// Owns the development-only SQLite database and process ownership behavior for the storage tracer.
module CacheStore =
    ...
    /// Acquires the sidecar byte-range lock before any schema or residue operation runs.
    ...
    /// Opens a connection and establishes the tracer's WAL, foreign-key, and bounded-busy settings.
    ...
    /// Creates the reset schema marker and rejects other development schema versions without migration.
    ...
    /// Retries only SQLite busy or locked operations within the declared bounded busy window.
    ...
    /// Opens the reset storage schema only after this process has acquired exclusive database ownership.
    ...
    /// Releases one store lease and unlocks the database only after every operation exits.
    ...
    /// Returns the settings verified while the current process acquired the store.
