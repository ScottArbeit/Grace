...
type QueueGatesConflict() =
    ...
    member _.RequiredActionsIncludePolicyConflictAndGate() =
        ...
        let types =
            actions
            |> List.map (fun action -> action.RequiredActionType)
    ...
    member _.GateResultsRespectPolicySnapshotAndRegistry() =
        ...
        let policyAttestation =
            Gates.runGate "policy" context
            |> Async.AwaitTask
    ...
    member _.BuildTestGateDefaultsToSkipped() =
        ...
        let attestation =
            Gates.runGate "build-test" context
            |> Async.AwaitTask
            |> Async.RunSynchronously
