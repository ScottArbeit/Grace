...
module Watch =
    ...
    let private materializationCompletedMarkerSidecars = Dictionary<string, DateTime>(StringComparer.OrdinalIgnoreCase)
    ...
    let private recordCurrentBranchMaterializationCompletedMarker markerFileName completedUtc =
        lock graceUpdateMarkerDeletionLock (fun () -> materializationCompletedMarkerSidecars[markerFileName] <- completedUtc)

    /// Consumes materialization marker authority so deletion cannot run branch-transition completion.
    let private tryConsumeCurrentBranchMaterializationCompletedMarker markerFileName completedUtc =
        lock graceUpdateMarkerDeletionLock (fun () ->
            match materializationCompletedMarkerSidecars.TryGetValue(markerFileName) with
            | true, recordedCompletedUtc when recordedCompletedUtc = completedUtc ->
                materializationCompletedMarkerSidecars.Remove(markerFileName)
                |> ignore

                true
            | _ -> false)

    /// Expires a materialization tag when a newer live marker has superseded the tagged marker instance.
    let private expireCurrentBranchMaterializationCompletedMarkerIfSuperseded markerFileName =
        try
            if File.Exists(markerFileName) then
                let markerLastWriteUtc = File.GetLastWriteTimeUtc(markerFileName)

                lock graceUpdateMarkerDeletionLock (fun () ->
                    match materializationCompletedMarkerSidecars.TryGetValue(markerFileName) with
                    | true, completedUtc when markerLastWriteUtc > completedUtc ->
                        materializationCompletedMarkerSidecars.Remove(markerFileName)
                        |> ignore
        ...
        task {
            ...
            let markerCreationResult =
                ...
                | :? IOException ->
                    Error(
                        GraceError.Create
                            "Current-branch remote materialization refused because the Grace update marker already exists; another Grace-owned working-tree update may be in progress."
                            (generateCorrelationId ())
                    )

            match markerCreationResult with
    ...
    /// Reports whether a current on-disk file still matches the file identity captured before remote materialization started.
    let private fileStillMatchesPreviousMaterializationAuthority (previousFile: LocalFileVersion) (currentFile: LocalFileVersion) =
        previousFile.Size = currentFile.Size
        && previousFile.Sha256Hash = currentFile.Sha256Hash
        && (String.IsNullOrWhiteSpace(string previousFile.Blake3Hash)
            || String.IsNullOrWhiteSpace(string currentFile.Blake3Hash)
            || previousFile.Blake3Hash = currentFile.Blake3Hash)

    /// Finds the previous GraceStatus file authority for a repository-relative materialization target.
    let private tryFindPreviousMaterializationFileAuthority (previousGraceStatus: GraceStatus) (relativePath: string) =
    ...
    let private verifyCurrentBranchMaterializationTargetAtOverwriteBoundaryWithAuthority
        ...
        task {
            ...
            if mutation.IsDirectory then
                ...
                if not (wasLastScanForDifferencesSuccessful ()) then
                    requestGraceWatchExplicitResync
                        $"Current-branch remote materialization could not complete final target verification for Reference {payload.ReferenceId}."
                    ...
                    return
                        Error(
                            GraceError.Create
                                $"Current-branch remote materialization refused to overwrite local edits at {relativePath}; Reference {payload.ReferenceId} was retired as stale behind local work."
                                correlationId
                        )
                else
                    return Ok()
            else
                match tryFindPreviousMaterializationFileAuthority previousGraceStatus relativePath with
                | Some previousFile ->
                    let currentFileInfo = FileInfo(fullPath)

                    if not currentFileInfo.Exists then
                        requestGraceWatchExplicitResync
                            $"Current-branch remote materialization detected a local delete at target {relativePath} before overwrite for Reference {payload.ReferenceId}."
                        ...
                        return
                            Error(
                                GraceError.Create
                                    $"Current-branch remote materialization refused to overwrite local deletion at {relativePath}; Reference {payload.ReferenceId} was retired as stale behind local work."
                                    correlationId
                            )
                    else
                        match! createLocalFileVersion currentFileInfo with
                        | Some currentFile when fileStillMatchesPreviousMaterializationAuthority previousFile currentFile -> return Ok()
                        | _ ->
                            requestGraceWatchExplicitResync
                                $"Current-branch remote materialization detected a local edit at target {relativePath} before overwrite for Reference {payload.ReferenceId}."
    ...
    let private processCurrentBranchReferenceMaterializationFromProvider
        (operations: CurrentBranchRemoteMaterializationOperations)
        recordPendingOnBlock
        (payloadProvider: unit -> CurrentBranchReferenceNotification seq)
        =
        task {
    ...
    let private processCurrentBranchReferenceMaterialization
        ...
        =
        processCurrentBranchReferenceMaterializationFromProvider operations recordPendingOnBlock (fun () -> payloads)
    ...
    let private retryPendingCurrentBranchMaterialization () =
        task {
            try
                let! _ =
                    processCurrentBranchReferenceMaterializationFromProvider (defaultCurrentBranchRemoteMaterializationOperations ()) false (fun () ->
                        pendingCurrentBranchMaterializationSnapshot ()
                        |> Seq.ofList)
    ...
    let internal retryPendingCurrentBranchMaterializationForWatchTests operations =
        task {
            try
                return!
                    processCurrentBranchReferenceMaterializationFromProvider operations false (fun () ->
                        pendingCurrentBranchMaterializationSnapshot ()
                        |> Seq.ofList)
            with
            | ex ->
    ...
    let OnGraceUpdateInProgressDeleted (args: FileSystemEventArgs) =
        if isUpdateMarkerPath args.FullPath then
            if not (File.Exists(args.FullPath)) then
                ...
                | Some completedUtc ->
                    if tryConsumeCurrentBranchMaterializationCompletedMarker args.FullPath completedUtc then
                ...
                expireCurrentBranchMaterializationCompletedMarkerIfSuperseded args.FullPath
                |> ignore
