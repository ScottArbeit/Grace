...
module Connect =
    ...
    module private Options = 
        ...
        let repositoryName = new Option<String>([|"--repositoryName"; "-n"|], IsRequired = false, Description = "The name of the repository.", Arity = ArgumentArity.ExactlyOne)
        ...
        let organizationId = new Option<String>("--organizationId", IsRequired = false, Description = "The repository's organization ID <Guid>.", Arity = ArgumentArity.ExactlyOne)
        let organizationName = new Option<String>("--organizationName", IsRequired = false, Description = "The repository's organization name.", Arity = ArgumentArity.ZeroOrOne)
        let correlationId = new Option<String>([|"--correlationId"; "-c"|], IsRequired = false, Description = "CorrelationId to track this command throughout Grace. [default: new Guid]", Arity = ArgumentArity.ExactlyOne)
        let serverAddress = new Option<String>([|"--serverAddress"; "-s"|], IsRequired = false, Description = "Address of the Grace server to connect to.", Arity = ArgumentArity.ExactlyOne)
        let retrieveDefaultBranch = new Option<bool>([|"--retrieveDefaultBranch"|], IsRequired = false, Description = "True to retrieve the default branch after connecting; false to connect but not download any files.", Arity = ArgumentArity.ExactlyOne)
    ...
    let private ValidateIncomingParameters (parseResult: ParseResult) commonParameters =
        ...
        let ``RepositoryId must be a non-empty Guid`` (parseResult: ParseResult, commonParameters: CommonParameters) =
            ...
            if parseResult.HasOption(Options.repositoryId) then
                match (Guid.isValidAndNotEmpty commonParameters.RepositoryId InvalidRepositoryId).Result with
        ...
        let ``RepositoryName must be valid`` (parseResult: ParseResult, commonParameters: CommonParameters) =
            if parseResult.HasOption(Options.repositoryName) then 
                match (String.isValidGraceName commonParameters.RepositoryName InvalidRepositoryName).Result with
                    | Ok result -> Result.Ok (parseResult, commonParameters)
                    | Error error -> Result.Error error
            else
                Result.Ok (parseResult, commonParameters)
        ...
        let ``OrganizationId must be a non-empty Guid`` (parseResult: ParseResult, commonParameters: CommonParameters) =
            ...
            if parseResult.HasOption(Options.organizationId) then
                match (Guid.isValidAndNotEmpty commonParameters.OrganizationId InvalidOrganizationId).Result with
                    | Ok result -> Result.Ok (parseResult, commonParameters)
                    | Error error -> Result.Error error
            else    
                Result.Ok (parseResult, commonParameters)
        ...
        let ``OrganizationName must be valid`` (parseResult: ParseResult, commonParameters: CommonParameters) =
            if parseResult.HasOption(Options.organizationName) then 
                match (String.isValidGraceName commonParameters.OrganizationName InvalidOrganizationName).Result with
                    | Ok result -> Result.Ok (parseResult, commonParameters)
                    | Error error -> Result.Error error
            else
                Result.Ok (parseResult, commonParameters)
