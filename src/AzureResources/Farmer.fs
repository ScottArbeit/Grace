...
let developmentApplicationIdentity =
    userAssignedIdentity {
        name "gracevcsDevelopmentUser"
        add_tags tags
    }

let developmentCosmosDB =
    cosmosDb {
...
let newStorageAccount (storageAccountName: String) =
    storageAccount {
...
let createStorageAccounts (region: Location) =
    List.init storageAccountCount (fun i -> i)
    |> List.map (fun i -> newStorageAccount $"{baseName}{region.ArmValue}{i:x2}")
...
let developmentDeployment region =
    arm {
        location region
...
let createSecretWithActivationDate (secretName: string) (secretValue: string) =
    secret {
        name secretName
...
let addKeyVaultSecrets (outputs: Map<string, string>) =
    keyVault {
        name "gracevcs-development"
        ...
        add_secret (
            createSecretWithActivationDate
                $"{storageAccountNames.[0]}AccountKey"
                (extractStorageKey outputs.[$"{storageAccountNames.[0]}PrimaryConnectionString"])
        )

        add_secret (
            createSecretWithActivationDate
                $"{storageAccountNames.[1]}AccountKey"
                (extractStorageKey outputs.[$"{storageAccountNames.[1]}PrimaryConnectionString"])
        )

        add_secret (
            createSecretWithActivationDate
                $"{storageAccountNames.[2]}AccountKey"
                (extractStorageKey outputs.[$"{storageAccountNames.[2]}PrimaryConnectionString"])
        )

        add_secret (
            createSecretWithActivationDate
                $"{storageAccountNames.[3]}AccountKey"
                (extractStorageKey outputs.[$"{storageAccountNames.[3]}PrimaryConnectionString"])
        )

        add_secret (
            createSecretWithActivationDate
                $"{storageAccountNames.[4]}AccountKey"
                (extractStorageKey outputs.[$"{storageAccountNames.[4]}PrimaryConnectionString"])
        )

        add_secret (
            createSecretWithActivationDate
                $"{storageAccountNames.[5]}AccountKey"
                (extractStorageKey outputs.[$"{storageAccountNames.[5]}PrimaryConnectionString"])
        )

        add_secret (
            createSecretWithActivationDate
                $"{storageAccountNames.[6]}AccountKey"
                (extractStorageKey outputs.[$"{storageAccountNames.[6]}PrimaryConnectionString"])
        )

        add_secret (
            createSecretWithActivationDate
                $"{storageAccountNames.[7]}AccountKey"
                (extractStorageKey outputs.[$"{storageAccountNames.[7]}PrimaryConnectionString"])
        )

        add_secret (
            createSecretWithActivationDate
                $"{storageAccountNames.[8]}AccountKey"
                (extractStorageKey outputs.[$"{storageAccountNames.[8]}PrimaryConnectionString"])
        )

        add_secret (
            createSecretWithActivationDate
                $"{storageAccountNames.[9]}AccountKey"
                (extractStorageKey outputs.[$"{storageAccountNames.[9]}PrimaryConnectionString"])
        )
...
let keyVaultDeployment outputs =
    arm {
...
|> Seq.iter (fun region ->
    ...
    let outputs =
        developmentDeployment region
        |> Deploy.execute "gracevcs-development" Deploy.NoParameters
