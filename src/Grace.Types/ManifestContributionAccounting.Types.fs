...
module ManifestContributionAccounting =
    ...
    module ExactRelationshipKey =
        ...
        let createPartitionKey partition =
            if isNull (box partition) then
                Error "Exact relationship partition must not be null."
            else
        ...
        /// Validates one complete relationship identity before any public projection or key derivation.
        let private validate relationship =
            if isNull (box relationship) then
                Error "Exact relationship must not be null."
            else
                match relationship with
                | ExactRelationship.ReferenceRoot relationship ->
                    if isNull (box relationship) then
                        Error "Reference-root relationship payload must not be null."
                    elif relationship.RepositoryId = Guid.Empty then
                ...
                | ExactRelationship.ParentChild relationship ->
                    if isNull (box relationship) then
                        Error "Parent-child relationship payload must not be null."
                    elif relationship.RepositoryId = Guid.Empty then
                ...
                | ExactRelationship.DirectoryVersionManifest relationship ->
                    if isNull (box relationship) then
                        Error "DirectoryVersion-manifest relationship payload must not be null."
                    elif relationship.RepositoryId = Guid.Empty then
                        ...
                        match tryEncodeComponent relationship.StoragePoolId, tryEncodeComponent relationship.ManifestAddress with
                        | Some _, Some _ -> Ok()
                        | _ -> Error "DirectoryVersion-manifest StoragePoolId and ManifestAddress must contain well-formed UTF-16."

        /// Returns validated partition dimensions used to enumerate the supplied exact relationship.
        let partition relationship =
            ...
            | Ok _ ->
                match relationship with
                | ExactRelationship.ReferenceRoot relationship ->
                    Ok(ExactRelationshipPartition.IncomingDirectoryVersion(relationship.RepositoryId, relationship.RootDirectoryVersionId))
                | ExactRelationship.ParentChild relationship ->
                    Ok(ExactRelationshipPartition.IncomingDirectoryVersion(relationship.RepositoryId, relationship.ChildDirectoryVersionId))
                | ExactRelationship.DirectoryVersionManifest relationship ->
                    Ok(ExactRelationshipPartition.Manifest(relationship.RepositoryId, relationship.StoragePoolId, relationship.ManifestAddress))

        /// Creates the canonical key for one exact relationship.
        let create relationship =
            match partition relationship with
            | Error error -> Error error
            | Ok partition ->
                match createPartitionKey partition with
                | Error error -> Error error
                | Ok partitionKey ->
                    match relationship with
                    | ExactRelationship.ReferenceRoot relationship ->
    ...
    module ExactRelationshipReadBound =
        ...
        /// Applies the positive finite range shared by construction and defensive unwrapping.
        let private validate maximumCount =
        ...
        let create maximumCount =
            ...
            |> Result.map ExactRelationshipReadBound

        /// Returns the positive finite count and rejects default or externally materialized invalid values.
        let value bound =
            if isNull (box bound) then
                ...
                match validate maximumCount with
                | Ok validMaximumCount -> validMaximumCount
                | Error error -> invalidArg (nameof bound) error
