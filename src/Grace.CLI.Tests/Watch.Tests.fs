...
module WatchTests =
    ...
    /// Reads the persisted Watch IPC directory ids into a deterministic set for assertions.
    let private readWatchStatusJsonDirectoryIds () =
        let json = File.ReadAllText(Services.IpcFileName())
        use document = JsonDocument.Parse(json)

        match document.RootElement.TryGetProperty("DirectoryIds") with
        | true, property ->
            property.EnumerateArray()
            |> Seq.map (fun directoryId -> DirectoryVersionId(directoryId.GetString()))
            |> Set.ofSeq
        | false, _ ->
            Assert.Fail($"Expected Watch IPC JSON property 'DirectoryIds'. JSON:{Environment.NewLine}{json}")
            Set.empty
    ...
    /// Wraps a Watch IPC status with inspection metadata used by current-branch materialization tests.
    ...
    /// Captures the active repository identity as materialization authority for focused Watch tests.
    let private currentMaterializationAuthorityFromStatus (status: GraceStatus) : Watch.CurrentBranchMaterializationAuthority =
        let current = Current()
    ...
    /// Builds injected materialization operations that record which side effects the Watch path requested.
    let private recordingMaterializationOperations
        (inspectionRef: Services.GraceWatchStatusInspection ref)
        (previousStatus: GraceStatus)
        (updatedStatus: GraceStatus)
        (directoryVersionDtos: DirectoryVersionDto array)
        =
        let fetchedRoots = ResizeArray<DirectoryVersionId>()
        let downloadBatches = ResizeArray<int>()
        let appliedPayloads = ResizeArray<CurrentBranchReferenceNotification>()
        let publishedStatuses = ResizeArray<GraceStatus>()
        let appliedStatusReachedDisk = ref false

        let operations: Watch.CurrentBranchRemoteMaterializationOperations =
            {
                InspectWatchStatus = fun () -> Task.FromResult(inspectionRef.Value)
                ReadGraceStatus =
                    fun () ->
                        if appliedStatusReachedDisk.Value then
                            Task.FromResult(updatedStatus)
                        else
                            Task.FromResult(previousStatus)
                GetDirectoryVersionsRecursive =
                    fun directoryVersionId _ ->
                        fetchedRoots.Add(directoryVersionId)
                        Task.FromResult<Result<DirectoryVersionDto array, GraceError>>(Ok(directoryVersionDtos))
                DownloadDirectoryFiles =
                    fun dtos _ ->
                        downloadBatches.Add(dtos.Length)
                        Task.FromResult<Result<unit, GraceError>>(Ok(()))
                ApplyRemoteDirectory =
                    fun payload _ _ _ _ ->
                        appliedPayloads.Add(payload)
                        appliedStatusReachedDisk.Value <- true
                        Task.FromResult<Result<GraceStatus, GraceError>>(Ok(updatedStatus))
                PublishWatchStatusRecordsSupersededRoot = false
                PublishWatchStatus =
                    fun _ status ->
        ...
        operations, fetchedRoots, downloadBatches, appliedPayloads, publishedStatuses

    /// Verifies that unsafe Watch states record a deferred remote sync without touching remote materialization clients.
    [<Test>]
    let ``current branch materialization blocks unsafe watch states before remote work`` () =
        withTempRepo (fun root ->
            ...
            let payload =
                currentBranchReferencePayload currentRepositoryId currentBranchId (Guid.NewGuid()) (Sha256Hash "remote-root") (Blake3Hash "remote-root-blake3")
            ...
            let cases =
                [|
                    "dirty", { cleanStatus with HasPendingWatchWork = true; IsWorkingTreeClean = false }, None
                    "applying", { cleanStatus with HasPendingWatchWork = true; IsWorkingTreeClean = true }, None
                    "scanning", { cleanStatus with DirectoryIds = HashSet<DirectoryVersionId>() }, None
                    "suspended", cleanStatus, Some Services.GraceWatchRuntimeMode.Suspended
                    "stale", { cleanStatus with UpdatedAt = staleObservation }, None
                    "root-mismatched", { cleanStatus with RootDirectory = Path.Combine(root, "other-root") }, None
                |]

            for caseName, status, persistedMode in cases do
                Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()

                let inspection = ref (materializationInspection persistedMode status)
                let previousStatus = GraceStatus.Default
                let updatedStatus = GraceStatus.Default

                let operations, fetchedRoots, downloadBatches, appliedPayloads, publishedStatuses =
                    recordingMaterializationOperations inspection previousStatus updatedStatus (Array.empty<DirectoryVersionDto>)
                ...
                pendingStatus.LatestDirectoryId
                |> should equal (Some payload.DirectoryId)

                pendingStatus.LatestReason
                |> should equal (Some decision.Reason)

                TestContext.WriteLine($"Blocked {caseName} with decision {decision.Reason}."))

    /// Verifies that a blocked remote sync is retained and retried when Watch returns to a clean boundary.
    [<Test>]
    let ``current branch materialization retries deferred reference after clean watch status`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            ...
            let payload =
                currentBranchReferencePayload
                    ...
                    currentBranchId
                    remoteRootId
                    (Sha256Hash "retry-remote-root")
                    (Blake3Hash "retry-remote-root-blake3")
            ...
            let directoryVersionDtos = [| DirectoryVersionDto.Default |]
            let inspection = ref (materializationInspection None dirtyStatus)

            let operations, fetchedRoots, downloadBatches, appliedPayloads, publishedStatuses =
                recordingMaterializationOperations inspection previousStatus updatedStatus directoryVersionDtos
            ...
            blockedDecision.NeedsMaterialization
            |> should equal false
            ...
            inspection.Value <- materializationInspection None cleanStatus

            let retryDecision =
                (Watch.retryPendingCurrentBranchMaterializationForWatchTests operations)
                    .GetAwaiter()
                    .GetResult()

            retryDecision.NeedsMaterialization
            |> should equal true
            ...
            downloadBatches
            |> Seq.toArray
            |> should equal [| directoryVersionDtos.Length |]
            ...
            let emptyRetryDecision =
                (Watch.retryPendingCurrentBranchMaterializationForWatchTests operations)
                    .GetAwaiter()
                    .GetResult()

            emptyRetryDecision.Reason
            |> should equal Services.LatestCurrentBranchReferenceDecisionReason.NoApplicableReference

            let clearedPendingStatus = Watch.pendingCurrentBranchMaterializationStatusForWatchTests ()

            clearedPendingStatus.Count |> should equal 0)

    /// Verifies that a failed remote apply keeps the pending sync recorded for a later retry.
    [<Test>]
    let ``current branch materialization keeps pending retry when remote apply fails`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            ...
            let payload =
                currentBranchReferencePayload
                    ...
                    currentBranchId
                    remoteRootId
                    (Sha256Hash "failed-remote-root")
                    (Blake3Hash "failed-remote-root-blake3")
            ...
            let failingOperations =
                { operations with
                    ApplyRemoteDirectory =
                        fun appliedPayload _ _ _ correlationId ->
                            appliedPayloads.Add(appliedPayload)

                            Task.FromResult<Result<GraceStatus, GraceError>>(
                                Error(GraceError.Create "simulated materialization boundary failure" correlationId)
                            )
                }

            let decision =
                (Watch.processCurrentBranchReferenceMaterializationForWatchTests failingOperations true [| payload |])
                    .GetAwaiter()
                    .GetResult()
    ...
    [<Test>]
    let ``current branch materialization detects changed local root before remote apply`` () =
        let previousStatus =
            { graceStatusTracking [| "before.txt" |] Array.empty<string> with
                RootDirectorySha256Hash = Sha256Hash "before-root"
                RootDirectoryBlake3Hash = Blake3Hash "before-root-blake3"
            }

        let advancedStatus =
            { graceStatusTracking [| "after.txt" |] Array.empty<string> with
                RootDirectorySha256Hash = Sha256Hash "after-root"
                RootDirectoryBlake3Hash = Blake3Hash "after-root-blake3"
            }

        let authority: Watch.CurrentBranchMaterializationAuthority =
            {
                RepositoryId = Current().RepositoryId
                RepositoryName = Current().RepositoryName
                BranchId = Current().BranchId
                ...
                RootDirectory = Current().RootDirectory
                ObjectDirectory = Current().ObjectDirectory
                RootDirectoryId = previousStatus.RootDirectoryId
                RootDirectorySha256Hash = previousStatus.RootDirectorySha256Hash
                RootDirectoryBlake3Hash = previousStatus.RootDirectoryBlake3Hash
            }

        Watch.currentBranchMaterializationRootMatchesAuthority authority previousStatus
        |> should equal true

        Watch.currentBranchMaterializationRootMatchesAuthority authority advancedStatus
        |> should equal false

    /// Verifies that a same-root branch switch during remote download blocks materialization before apply.
    [<Test>]
    let ``current branch materialization revalidates branch authority after remote download`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            ...
            let payload =
                currentBranchReferencePayload
                    ...
                    currentBranchId
                    remoteRootId
                    (Sha256Hash "same-root-switch")
                    (Blake3Hash "same-root-switch-blake3")
            ...
            let switchingOperations =
                { operations with
                    DownloadDirectoryFiles =
                        fun dtos correlationId ->
                            configureCurrentWatchIdentity root "current-repo" "other-branch"
                            |> ignore

                            operations.DownloadDirectoryFiles dtos correlationId
                }

            let decision =
                (Watch.processCurrentBranchReferenceMaterializationForWatchTests switchingOperations true [| payload |])
                    .GetAwaiter()
                    .GetResult()
    ...
    /// Verifies that durable status advancement between decision and marker-time write blocks rollback.
    [<Test>]
    let ``current branch materialization revalidates durable clean root before write`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()

            let currentRepositoryId, currentBranchId = configureCurrentWatchIdentity root "current-repo" "current-branch"
            let localRootId = Guid.NewGuid()
            let advancedRootId = Guid.NewGuid()
            let remoteRootId = Guid.NewGuid()
            let cleanStatus = liveWatchStatus localRootId
            ...
            let advancedStatus =
                { previousStatus with
                    RootDirectoryId = advancedRootId
                    RootDirectorySha256Hash = Sha256Hash "advanced-local-root"
                    RootDirectoryBlake3Hash = Blake3Hash "advanced-local-root-blake3"
                }
            ...
            let payload =
                currentBranchReferencePayload
                    ...
                    currentBranchId
                    remoteRootId
                    (Sha256Hash "remote-after-advance")
                    (Blake3Hash "remote-after-advance-blake3")
            ...
            let operations: Watch.CurrentBranchRemoteMaterializationOperations =
                {
                    InspectWatchStatus = fun () -> Task.FromResult(inspection.Value)
                    ReadGraceStatus =
                        fun () ->
                            incr readCount

                            if readCount.Value = 1 then
                                Task.FromResult(previousStatus)
                            else
                                Task.FromResult(advancedStatus)
                    GetDirectoryVersionsRecursive =
                        fun _ _ -> Task.FromResult<Result<DirectoryVersionDto array, GraceError>>(Ok [| DirectoryVersionDto.Default |])
                    DownloadDirectoryFiles = fun _ _ -> Task.FromResult<Result<unit, GraceError>>(Ok())
                    ApplyRemoteDirectory =
                        fun appliedPayload _ _ _ _ ->
                            appliedPayloads.Add(appliedPayload)
                            Task.FromResult<Result<GraceStatus, GraceError>>(Ok advancedStatus)
                    PublishWatchStatusRecordsSupersededRoot = false
                    PublishWatchStatus = fun _ _ -> Task.FromException<unit>(InvalidOperationException("stale materialization must not publish"))
                }
            ...
            readCount.Value |> should equal 2
            appliedPayloads.Count |> should equal 0)

    /// Verifies that a newer deferred root wins over an older delayed live notification.
    [<Test>]
    let ``current branch materialization evaluates deferred payloads before live notification`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            ...
            let olderPayload =
                currentBranchReferencePayload
                    ...
                    currentBranchId
                    olderRootId
                    (Sha256Hash "live-delayed-older-root")
                    (Blake3Hash "live-delayed-older-root-blake3")
            ...
            (Watch.processCurrentBranchReferenceMaterializationForWatchTests operations true [| olderPayload |])
                .GetAwaiter()
                .GetResult()
            |> ignore
        ...
        withTempRepo (fun root ->
            ...
            let olderPayload =
                currentBranchReferencePayload
                    ...
                    currentBranchId
                    olderRootId
                    (Sha256Hash "live-delayed-older-retry-root")
                    (Blake3Hash "live-delayed-older-retry-root-blake3")
            ...
            let newerPayload =
                currentBranchReferencePayload
                    ...
                    currentBranchId
                    newerRootId
                    (Sha256Hash "retry-deferred-newer-root")
                    (Blake3Hash "retry-deferred-newer-root-blake3")
    ...
    /// Verifies that branch authority is rechecked after apply and before IPC publication.
    [<Test>]
    let ``current branch materialization rejects branch switch before status publish`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            Watch.clearGraceWatchResyncPendingForWatchTests ()
            ...
            let payload =
                currentBranchReferencePayload
                    ...
                    currentBranchId
                    remoteRootId
                    (Sha256Hash "publish-switch-root")
                    (Blake3Hash "publish-switch-root-blake3")
            ...
            let operations, _, _, appliedPayloads, publishedStatuses =
                recordingMaterializationOperations inspection previousStatus materializedStatus [| DirectoryVersionDto.Default |]

            let switchingOperations =
                { operations with
                    ApplyRemoteDirectory =
                        fun appliedPayload authority previous directoryVersions correlationId ->
                            task {
                                let! result = operations.ApplyRemoteDirectory appliedPayload authority previous directoryVersions correlationId
            ...
            appliedPayloads.Count |> should equal 1
            publishedStatuses.Count |> should equal 0
    ...
    /// Verifies that the first clean publication on a new branch retires that branch's deferred remote payloads.
    [<Test>]
    let ``current branch materialization retires pending remote after branch switch clean publication`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()

            let _, _ = configureCurrentWatchIdentity root "current-repo" "previous-branch"
            let previousBranchRootId = Guid.NewGuid()

            Watch.recordPublishedCurrentBranchCleanRootForWatchTests
                { GraceStatus.Default with
                    RootDirectoryId = previousBranchRootId
                    RootDirectorySha256Hash = Sha256Hash "previous-branch-root"
                    RootDirectoryBlake3Hash = Blake3Hash "previous-branch-root-blake3"
                }

            let currentRepositoryId, currentBranchId = configureCurrentWatchIdentity root "current-repo" "current-branch"
            let queuedRootId = Guid.NewGuid()
            let localSaveRootId = Guid.NewGuid()
            ...
            let queuedPayload =
                currentBranchReferencePayload
                    ...
                    currentBranchId
                    queuedRootId
                    (Sha256Hash "queued-before-new-branch-local-save")
                    (Blake3Hash "queued-before-new-branch-local-save-blake3")
            ...
            Watch.recordPublishedCurrentBranchCleanRootForWatchTests
                { GraceStatus.Default with
                    RootDirectoryId = localSaveRootId
                    RootDirectorySha256Hash = Sha256Hash "new-branch-local-save"
                    RootDirectoryBlake3Hash = Blake3Hash "new-branch-local-save-blake3"
                }

            let cleanStatusAfterLocalSave =
                { liveWatchStatus localSaveRootId with
                    RootDirectorySha256Hash = Sha256Hash "new-branch-local-save"
                    RootDirectoryBlake3Hash = Blake3Hash "new-branch-local-save-blake3"
                    DirectoryIds = HashSet<DirectoryVersionId>([| localSaveRootId |])
                }

            let localSaveStatus =
                { GraceStatus.Default with
                    RootDirectoryId = localSaveRootId
                    RootDirectorySha256Hash = cleanStatusAfterLocalSave.RootDirectorySha256Hash
                    RootDirectoryBlake3Hash = cleanStatusAfterLocalSave.RootDirectoryBlake3Hash
                }
            ...
            (Watch.processCurrentBranchReferenceMaterializationForWatchTests operations true [| queuedPayload |])
                .GetAwaiter()
                .GetResult()
            |> ignore
    ...
    /// Verifies that marker-time local-root drift retires the queued remote payload.
    [<Test>]
    let ``current branch materialization marker root mismatch retires pending payload`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            ...
            let payload =
                currentBranchReferencePayload
                    ...
                    currentBranchId
                    remoteRootId
                    (Sha256Hash "marker-root-stale-remote")
                    (Blake3Hash "marker-root-stale-remote-blake3")
            ...
            let advancedStatus =
                { previousStatus with
                    RootDirectoryId = advancedRootId
                    RootDirectorySha256Hash = Sha256Hash "marker-root-advanced-local"
                    RootDirectoryBlake3Hash = Blake3Hash "marker-root-advanced-local-blake3"
                }
    ...
    /// Verifies that contained directory membership does not make a same-branch Reference stale.
    [<Test>]
    let ``current branch materialization applies reference whose root is contained in clean tree`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()

            let currentRepositoryId, currentBranchId = configureCurrentWatchIdentity root "current-repo" "current-branch"
            let containedRootId = Guid.NewGuid()
            let currentRootId = Guid.NewGuid()

            let cleanStatus = { liveWatchStatus currentRootId with DirectoryIds = HashSet<DirectoryVersionId>([| currentRootId; containedRootId |]) }
            ...
            let payload =
                currentBranchReferencePayload
                    ...
                    currentBranchId
                    containedRootId
                    (Sha256Hash "contained-root")
                    (Blake3Hash "contained-root-blake3")

            let previousStatus =
                { GraceStatus.Default with
                    RootDirectoryId = currentRootId
                    RootDirectorySha256Hash = cleanStatus.RootDirectorySha256Hash
                    RootDirectoryBlake3Hash = cleanStatus.RootDirectoryBlake3Hash
                }
            ...
            let updatedStatus =
                { previousStatus with
                    RootDirectoryId = containedRootId
                    RootDirectorySha256Hash = payload.Sha256Hash
                    RootDirectoryBlake3Hash = payload.Blake3Hash
                }
        ...
        withTempRepo (fun root ->
            ...
            let delayedOlderPayload =
                { currentBranchReferencePayload
                      ...
                      olderLocalStatus.RootDirectoryBlake3Hash with
    ...
    /// Verifies that fallback downloads fail before mutation when Current points at a different object cache.
    [<Test>]
    let ``current branch materialization fallback download refuses drifted object cache`` () =
        withTempRepo (fun root ->
            configureCurrentWatchIdentity root "current-repo" "current-branch"
            |> ignore

            let capturedObjectDirectory = Current().ObjectDirectory
            let driftedObjectDirectory = Path.Combine(root, ".grace", "drifted-objects")
            let remoteRootId = Guid.NewGuid()
            let relativePath = RelativePath "object-cache-drift.txt"
            let workingFilePath = Path.Combine(root, string relativePath)
            let previousBytes = Encoding.UTF8.GetBytes("previous file content")
            let remoteBytes = Encoding.UTF8.GetBytes("remote bytes should not be copied")
            ...
            let previousRoot =
                LocalDirectoryVersion.CreateWithHashes
                    ...
                    (Current().RepositoryId)
                    Constants.RootDirectoryPath
                    (Sha256Hash "object-cache-drift-local-root")
                    (Blake3Hash "object-cache-drift-local-root-blake3")
                    (List<DirectoryVersionId>())
                    (List<LocalFileVersion>([| previousFile |]))
                    previousFile.Size
                    DateTime.UtcNow
            ...
            previousIndex.TryAdd(previousRoot.DirectoryVersionId, previousRoot)
            |> ignore
            ...
            let remoteFile =
                FileVersion.CreateWithHashes
                    relativePath
                    (Sha256Hash "object-cache-drift-remote-file")
                    (Blake3Hash "object-cache-drift-remote-file-blake3")
                    String.Empty
                    false
                    (int64 remoteBytes.Length)
            ...
            let remoteRoot =
                DirectoryVersion.CreateWithHashes
                    ...
                    (Current().RepositoryId)
                    Constants.RootDirectoryPath
                    (Sha256Hash "object-cache-drift-remote-root")
                    (Blake3Hash "object-cache-drift-remote-root-blake3")
                    (List<DirectoryVersionId>())
                    (List<FileVersion>([| remoteFile |]))
                    (int64 remoteBytes.Length)
            ...
            let downloadCalls = ref 0
            let originalObjectDirectory = Current().ObjectDirectory
            Current().ObjectDirectory <- driftedObjectDirectory

            try
                Services.setDownloadFileVersionsForWorkingDirectoryUpdateForTests (fun _ _ _ ->
                    incr downloadCalls
                    Task.FromResult<Result<unit, string>>(Ok()))

                let result =
                    (Services.updateWorkingDirectoryWithTargetGuardAtRoot
                        root
                        capturedObjectDirectory
                        previousStatus
                        updatedStatus
                        [| remoteDto |]
                        (generateCorrelationId ())
                        (fun _ -> Task.FromResult<Result<unit, GraceError>>(Ok())))
                        .GetAwaiter()
                        .GetResult()

                result.IsError |> should equal true
                downloadCalls.Value |> should equal 0

                File.ReadAllBytes(workingFilePath)
                |> should equal previousBytes
            finally
                Current().ObjectDirectory <- originalObjectDirectory
                Services.resetDownloadFileVersionsForWorkingDirectoryUpdateForTests ())

    /// Verifies that durable-only pending evidence is published through the captured branch IPC writer.
    [<Test>]
    let ``current branch materialization durable fallback uses captured ipc writer`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()

            let branchARepositoryId, branchAId = configureCurrentWatchIdentity root "current-repo" "branch-a"
            let branchAIpcFileName = Services.IpcFileName()
            let branchAStatus = graceStatusTracking [| "captured-branch.txt" |] Array.empty<string>
            let branchAWriter = Services.updateGraceWatchInterprocessFileForIdentity branchARepositoryId "current-repo" root branchAId "branch-a"
            ...
            try
                ...
                let current = Current()
                current.RepositoryName <- "current-repo"
                current.BranchId <- Guid.NewGuid()
                current.BranchName <- "branch-b"

                let branchBIpcFileName = Services.IpcFileName()

                if File.Exists(branchAIpcFileName) then File.Delete(branchAIpcFileName)
                if File.Exists(branchBIpcFileName) then File.Delete(branchBIpcFileName)

                let result =
                    (Watch.publishCurrentBranchMaterializedStatusWithWriterAndInspectorForWatchTests branchAStatus branchAWriter (fun () ->
                        Services.inspectGraceWatchStatusForIdentity branchARepositoryId "current-repo" root branchAId "branch-a"))
                        .GetAwaiter()
                        .GetResult()

                result.IsOk |> should equal true

                File.Exists(branchAIpcFileName)
                |> should equal true

                File.Exists(branchBIpcFileName)
                |> should equal false

                use document = JsonDocument.Parse(File.ReadAllText(branchAIpcFileName))

                document
                    .RootElement
                    .GetProperty("RepositoryId")
                    .GetGuid()
                |> should equal branchARepositoryId
                ...
                document
                    .RootElement
                    .GetProperty("HasPendingWatchWork")
                    .GetBoolean()
                |> should equal true
            finally
                Watch.resetWatchJournalClientsForWatchTests ())

    /// Verifies that durable status writes fail closed instead of following Current() after a root switch.
    [<Test>]
    let ``current branch materialization durable status write uses captured authority`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            ...
            let authorityStatus =
                { graceStatusTracking [| "captured-before.txt" |] Array.empty<string> with
                    RootDirectorySha256Hash = Sha256Hash "captured-before-root"
                    RootDirectoryBlake3Hash = Blake3Hash "captured-before-root-blake3"
                }

            let authority = currentMaterializationAuthorityFromStatus authorityStatus

            let materializedStatus =
                { graceStatusTracking [| "captured-after.txt" |] Array.empty<string> with
                    RootDirectorySha256Hash = Sha256Hash "captured-after-root"
                    RootDirectoryBlake3Hash = Blake3Hash "captured-after-root-blake3"
                }
            ...
            let payload =
                currentBranchReferencePayload
                    repositoryId
                    branchId
                    materializedStatus.RootDirectoryId
                    materializedStatus.RootDirectorySha256Hash
                    materializedStatus.RootDirectoryBlake3Hash

            let authorityDbPath = Path.Combine(root, Constants.GraceConfigDirectory, Constants.GraceLocalStateDbFileName)

            (LocalStateDb.replaceStatusSnapshot authorityDbPath authorityStatus)
                .GetAwaiter()
                .GetResult()

            let switchedRoot = Path.Combine(root, "switched-root")

            Directory.CreateDirectory(Path.Combine(switchedRoot, Constants.GraceConfigDirectory))
            |> ignore

            writeRepositoryConfiguration switchedRoot repositoryId "current-repo" branchId "current-branch"
            Environment.CurrentDirectory <- switchedRoot
            resetConfiguration ()
            Current() |> ignore

            let switchedStatus =
                { graceStatusTracking [| "switched-current.txt" |] Array.empty<string> with
                    RootDirectorySha256Hash = Sha256Hash "switched-current-root"
                    RootDirectoryBlake3Hash = Blake3Hash "switched-current-root-blake3"
                }

            let switchedDbPath = Current().GraceStatusFile

            (LocalStateDb.replaceStatusSnapshot switchedDbPath switchedStatus)
                .GetAwaiter()
                .GetResult()

            let result =
                (Watch.writeMaterializedCurrentBranchDurableStateForAuthorityForWatchTests payload authority materializedStatus (generateCorrelationId ()))
                    .GetAwaiter()
                    .GetResult()

            result.IsError |> should equal true

            (LocalStateDb.readStatusSnapshot authorityDbPath)
                .GetAwaiter()
                .GetResult()
                .RootDirectoryId
            |> should equal authorityStatus.RootDirectoryId

            (LocalStateDb.readStatusSnapshot switchedDbPath)
                .GetAwaiter()
                .GetResult()
                .RootDirectoryId
            |> should equal switchedStatus.RootDirectoryId)

    /// Verifies that captured-authority publication cannot record a materialized root against a branch selected later.
    [<Test>]
    let ``current branch materialization captured publication does not retire switched branch pending remote`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            Watch.clearPendingWatchWorkForTests ()
            Services.setGraceWatchHasPendingWorkForStatus false

            let repositoryId, branchAId = configureCurrentWatchIdentity root "current-repo" "branch-a"

            let branchAStatus =
                { graceStatusTracking [| "branch-a.txt" |] Array.empty<string> with
                    RootDirectoryId = Guid.NewGuid()
                    RootDirectorySha256Hash = Sha256Hash "captured-branch-a-root"
                    RootDirectoryBlake3Hash = Blake3Hash "captured-branch-a-root-blake3"
                }

            let branchAAuthority = currentMaterializationAuthorityFromStatus branchAStatus
            let observedGeneration = Watch.currentPublishedCurrentBranchCleanRootGenerationForWatchTests ()
            let branchBId = Guid.NewGuid()
            let current = Current()
            ...
            let branchBPayload =
                currentBranchReferencePayload
                    ...
                    branchBId
                    (Guid.NewGuid())
                    (Sha256Hash "captured-publication-branch-b-remote")
                    (Blake3Hash "captured-publication-branch-b-remote-blake3")

            let result =
                (Watch.publishCurrentBranchMaterializedStatusForAuthorityForWatchTests branchAAuthority branchAStatus)
                    .GetAwaiter()
                    .GetResult()
            ...
            pendingStatus.LatestDirectoryId
            |> should equal (Some branchBPayload.DirectoryId))
    ...
    /// Verifies that a tracked directory can be preflighted as an authorized remote file replacement.
    [<Test>]
    let ``current branch materialization target guard allows tracked directory at file replacement target`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            Watch.clearGraceWatchResyncPendingForWatchTests ()
            ...
            let currentRepositoryId, currentBranchId = configureCurrentWatchIdentity root "current-repo" "current-branch"
            let localRootId = Guid.NewGuid()
            let trackedDirectoryId = Guid.NewGuid()
            let remoteRootId = Guid.NewGuid()
            let swappedPath = RelativePath "tracked-dir-file-guard"
            ...
            let remoteBytes = Encoding.UTF8.GetBytes("remote file replaces tracked directory through guard")

            Directory.CreateDirectory(swappedDirectoryPath)
            |> ignore

            File.WriteAllText(nestedPath, "tracked child")
            ...
            let previousDirectory =
                LocalDirectoryVersion.CreateWithHashes
                    ...
                    currentRepositoryId
                    swappedPath
                    (Sha256Hash "tracked-dir-file-guard-directory")
                    (Blake3Hash "tracked-dir-file-guard-directory-blake3")
                    (List<DirectoryVersionId>())
            ...
            let previousRoot =
                LocalDirectoryVersion.CreateWithHashes
                    ...
                    currentRepositoryId
                    Constants.RootDirectoryPath
                    (Sha256Hash "tracked-dir-file-guard-local-root")
                    (Blake3Hash "tracked-dir-file-guard-local-root-blake3")
                    (List<DirectoryVersionId>([| trackedDirectoryId |]))
                    (List<LocalFileVersion>())
                    previousChild.Size
                    DateTime.UtcNow
            ...
            previousIndex.TryAdd(trackedDirectoryId, previousDirectory)
            |> ignore
            ...
            let remoteFile =
                FileVersion.CreateWithHashes
                    swappedPath
                    (Sha256Hash "tracked-dir-file-guard-remote-file")
                    (Blake3Hash "tracked-dir-file-guard-remote-file-blake3")
                    String.Empty
                    false
                    (int64 remoteBytes.Length)
            ...
            let remoteRoot =
                DirectoryVersion.CreateWithHashes
                    ...
                    currentRepositoryId
                    Constants.RootDirectoryPath
                    (Sha256Hash "tracked-dir-file-guard-remote-root")
                    (Blake3Hash "tracked-dir-file-guard-remote-root-blake3")
                    (List<DirectoryVersionId>())
                    (List<FileVersion>([| remoteFile |]))
                    (int64 remoteBytes.Length)
    ...
    /// Verifies that a local file created after preflight at a remote-create target fails closed.
    [<Test>]
    let ``current branch materialization target guard rejects local file created after preflight`` () =
        withTempRepo (fun root ->
            configureCurrentWatchIdentity root "current-repo" "current-branch"
            |> ignore

            let remoteRootId = Guid.NewGuid()
            let relativePath = RelativePath "raced-create.txt"
            let targetPath = Path.Combine(root, string relativePath)
            let remoteBytes = Encoding.UTF8.GetBytes("remote content")
            let localBytes = Encoding.UTF8.GetBytes("local content created after preflight")

            let remoteFile =
                FileVersion.CreateWithHashes
                    relativePath
                    (Sha256Hash "raced-create-remote-file")
                    (Blake3Hash "raced-create-remote-file-blake3")
                    String.Empty
                    false
                    (int64 remoteBytes.Length)
            ...
            let remoteRoot =
                DirectoryVersion.CreateWithHashes
                    ...
                    (Current().RepositoryId)
                    Constants.RootDirectoryPath
                    (Sha256Hash "raced-create-remote-root")
                    (Blake3Hash "raced-create-remote-root-blake3")
                    (List<DirectoryVersionId>())
                    (List<FileVersion>([| remoteFile |]))
                    (int64 remoteBytes.Length)
            ...
            let result =
                (Services.updateWorkingDirectoryWithTargetGuard GraceStatus.Default updatedStatus [| remoteDto |] (generateCorrelationId ()) (fun mutation ->
                    if
                        not mutation.IsDirectory
                        && not mutation.DeletesExistingContent
                        && String.Equals(Path.GetFullPath(mutation.FullPath), Path.GetFullPath(targetPath), StringComparison.OrdinalIgnoreCase)
                        && not (File.Exists(targetPath))
                    then
            ...
            |> should equal localBytes)

    /// Verifies that remote apply uses captured rooted paths even if Current changes before file copy.
    [<Test>]
    let ``current branch materialization rooted apply ignores later current root drift`` () =
        withTempRepo (fun root ->
            configureCurrentWatchIdentity root "current-repo" "current-branch"
            |> ignore

            let capturedObjectDirectory = Current().ObjectDirectory
            let otherRoot = Path.Combine(Path.GetTempPath(), $"grace-root-drift-{Guid.NewGuid():N}")
            Directory.CreateDirectory(otherRoot) |> ignore

            try
                let remoteRootId = Guid.NewGuid()
                let relativePath = RelativePath "rooted-apply.txt"
                let capturedTargetPath = Path.Combine(root, string relativePath)
                let driftedTargetPath = Path.Combine(otherRoot, string relativePath)
                let remoteBytes = Encoding.UTF8.GetBytes("remote rooted content")

                let remoteFile =
                    FileVersion.CreateWithHashes
                        relativePath
                        (Sha256Hash "rooted-apply-remote-file")
                        (Blake3Hash "rooted-apply-remote-file-blake3")
                        String.Empty
                        false
                        (int64 remoteBytes.Length)
                ...
                let remoteRoot =
                    DirectoryVersion.CreateWithHashes
                        ...
                        (Current().RepositoryId)
                        Constants.RootDirectoryPath
                        (Sha256Hash "rooted-apply-remote-root")
                        (Blake3Hash "rooted-apply-remote-root-blake3")
                        (List<DirectoryVersionId>())
                        (List<FileVersion>([| remoteFile |]))
                        (int64 remoteBytes.Length)
                ...
                Current().RootDirectory <- otherRoot
                ...
                File.ReadAllBytes(capturedTargetPath)
                |> should equal remoteBytes

                File.Exists(driftedTargetPath)
                |> should equal false
            finally
                if Directory.Exists(otherRoot) then Directory.Delete(otherRoot, true))

    /// Verifies that directory-target verification scans the captured authority root after Current drifts.
    [<Test>]
    let ``current branch materialization directory target guard scans captured authority root`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            Watch.clearGraceWatchResyncPendingForWatchTests ()
            ...
            let trackedDirectoryId = Guid.NewGuid()
            let remoteRootId = Guid.NewGuid()
            let directoryPath = RelativePath "captured-scan-dir"
            let targetDirectoryPath = Path.Combine(root, string directoryPath)
            let childPath = Path.Combine(targetDirectoryPath, "child.txt")
            let otherRoot = Path.Combine(Path.GetTempPath(), $"grace-scan-drift-{Guid.NewGuid():N}")

            Directory.CreateDirectory(targetDirectoryPath)
            |> ignore

            File.WriteAllText(childPath, "clean child")

            let previousChild =
                (Services.createLocalFileVersion (FileInfo(childPath)))
                    .GetAwaiter()
                    .GetResult()
                    .Value
            ...
            let previousDirectory =
                LocalDirectoryVersion.CreateWithHashes
                    ...
                    currentRepositoryId
                    directoryPath
                    (Sha256Hash "captured-scan-directory")
                    (Blake3Hash "captured-scan-directory-blake3")
                    (List<DirectoryVersionId>())
                    (List<LocalFileVersion>([| previousChild |]))
                    previousChild.Size
                    DateTime.UtcNow
            ...
            let previousRoot =
                LocalDirectoryVersion.CreateWithHashes
                    ...
                    currentRepositoryId
                    Constants.RootDirectoryPath
                    (Sha256Hash "captured-scan-root")
                    (Blake3Hash "captured-scan-root-blake3")
                    (List<DirectoryVersionId>([| trackedDirectoryId |]))
                    (List<LocalFileVersion>())
                    previousChild.Size
                    DateTime.UtcNow
            ...
            let authority = currentMaterializationAuthorityFromStatus previousStatus
            let originalRootDirectory = Current().RootDirectory

            try
                let otherDirectoryPath = Path.Combine(otherRoot, string directoryPath)

                Directory.CreateDirectory(otherDirectoryPath)
                |> ignore

                File.WriteAllText(Path.Combine(otherDirectoryPath, "child.txt"), "clean child")
                File.WriteAllText(childPath, "racing edit under captured root")
                File.SetLastWriteTimeUtc(childPath, previousChild.LastWriteTimeUtc.AddMinutes(1.0))
                ...
                let payload =
                    currentBranchReferencePayload
                        ...
                        currentBranchId
                        remoteRootId
                        (Sha256Hash "captured-scan-remote-root")
                        (Blake3Hash "captured-scan-remote-root-blake3")

                Watch.rememberPendingCurrentBranchMaterializationForWatchTests
                    payload
                    Services.LatestCurrentBranchReferenceDecisionReason.RemoteMaterializationRequired

                let mutation: Services.WorkingDirectoryTargetMutation = { FullPath = targetDirectoryPath; IsDirectory = true; DeletesExistingContent = true }

                let result =
                    (Watch.verifyCurrentBranchMaterializationTargetAtOverwriteBoundaryWithAuthorityForWatchTests
                        payload
                        authority
                        previousStatus
                        (generateCorrelationId ())
                ...
                Current().RootDirectory <- originalRootDirectory

                if Directory.Exists(otherRoot) then Directory.Delete(otherRoot, true))

    /// Verifies that a remote file can replace a tracked directory at the same path.
    [<Test>]
    let ``current branch materialization applies directory to file type swap`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            ...
            let remoteRootId = Guid.NewGuid()
            let swappedPath = RelativePath "type-swap"
            let swappedDirectoryPath = Path.Combine(root, string swappedPath)
            let nestedPath = Path.Combine(swappedDirectoryPath, "old-child.txt")
            let remoteBytes = Encoding.UTF8.GetBytes("remote file replaces directory")
            ...
            let previousDirectory =
                LocalDirectoryVersion.CreateWithHashes
                    ...
                    currentRepositoryId
                    swappedPath
                    (Sha256Hash "type-swap-local-directory")
                    (Blake3Hash "type-swap-local-directory-blake3")
                    (List<DirectoryVersionId>())
                    (List<LocalFileVersion>([| previousChild |]))
                    previousChild.Size
                    DateTime.UtcNow
            ...
            let previousRoot =
                LocalDirectoryVersion.CreateWithHashes
                    ...
                    currentRepositoryId
                    Constants.RootDirectoryPath
                    (Sha256Hash "type-swap-dir-file-local-root")
                    (Blake3Hash "type-swap-dir-file-local-root-blake3")
            ...
            let remoteFile =
                FileVersion.CreateWithHashes
                    swappedPath
                    (Sha256Hash "type-swap-remote-file")
                    (Blake3Hash "type-swap-remote-file-blake3")
                    String.Empty
                    false
                    (int64 remoteBytes.Length)
            ...
            let remoteRoot =
                DirectoryVersion.CreateWithHashes
                    ...
                    currentRepositoryId
                    Constants.RootDirectoryPath
                    (Sha256Hash "type-swap-dir-file-remote-root")
                    (Blake3Hash "type-swap-dir-file-remote-root-blake3")
                    (List<DirectoryVersionId>())
                    (List<FileVersion>([| remoteFile |]))
                    (int64 remoteBytes.Length)
    ...
    [<Test>]
    let ``current branch materialization applies file to directory type swap`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            ...
            let swappedPath = RelativePath "file-to-directory"
            let swappedFilePath = Path.Combine(root, string swappedPath)

            File.WriteAllText(swappedFilePath, "old tracked file")

            let previousFile =
                (Services.createLocalFileVersion (FileInfo(swappedFilePath)))
                    .GetAwaiter()
                    .GetResult()
                    .Value
            ...
            let previousRoot =
                LocalDirectoryVersion.CreateWithHashes
                    ...
                    currentRepositoryId
                    Constants.RootDirectoryPath
                    (Sha256Hash "type-swap-file-dir-local-root")
                    (Blake3Hash "type-swap-file-dir-local-root-blake3")
                    (List<DirectoryVersionId>())
                    (List<LocalFileVersion>([| previousFile |]))
                    previousFile.Size
                    DateTime.UtcNow
            ...
            let remoteDirectory =
                DirectoryVersion.CreateWithHashes
                    ...
                    currentRepositoryId
                    swappedPath
                    (Sha256Hash "type-swap-remote-directory")
                    (Blake3Hash "type-swap-remote-directory-blake3")
                    (List<DirectoryVersionId>())
                    (List<FileVersion>())
                    0L
            ...
            let remoteRoot =
                DirectoryVersion.CreateWithHashes
                    ...
                    currentRepositoryId
                    Constants.RootDirectoryPath
                    (Sha256Hash "type-swap-file-dir-remote-root")
                    (Blake3Hash "type-swap-file-dir-remote-root-blake3")
                    (List<DirectoryVersionId>([| remoteDirectoryId |]))
                    (List<FileVersion>())
                    0L

            let remoteDtos =
                [|
                    { DirectoryVersionDto.Default with DirectoryVersion = remoteRoot; RecursiveSize = 0L }
                    { DirectoryVersionDto.Default with DirectoryVersion = remoteDirectory; RecursiveSize = 0L }
                |]

            let updatedIndex = GraceIndex()
            let updatedRoot = remoteRoot.ToLocalDirectoryVersion DateTime.UtcNow
            let updatedDirectory = remoteDirectory.ToLocalDirectoryVersion DateTime.UtcNow

            updatedIndex.TryAdd(remoteRootId, updatedRoot)
            |> ignore

            updatedIndex.TryAdd(remoteDirectoryId, updatedDirectory)
            |> ignore
            ...
            File.Exists(swappedFilePath) |> should equal false

            Directory.Exists(swappedFilePath)
            |> should equal true)

    /// Verifies that a remote directory can replace a tracked file without treating the authorized delete as local work.
    [<Test>]
    let ``current branch materialization target guard allows file to directory type swap`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            Watch.clearGraceWatchResyncPendingForWatchTests ()
            ...
            let remoteRootId = Guid.NewGuid()
            let remoteDirectoryId = Guid.NewGuid()
            let swappedPath = RelativePath "guarded-file-to-directory"
            let swappedFilePath = Path.Combine(root, string swappedPath)
            ...
            let previousRoot =
                LocalDirectoryVersion.CreateWithHashes
                    ...
                    currentRepositoryId
                    Constants.RootDirectoryPath
                    (Sha256Hash "guarded-file-dir-local-root")
                    (Blake3Hash "guarded-file-dir-local-root-blake3")
                    (List<DirectoryVersionId>())
                    (List<LocalFileVersion>([| previousFile |]))
                    previousFile.Size
                    DateTime.UtcNow
            ...
            let remoteDirectory =
                DirectoryVersion.CreateWithHashes
                    ...
                    currentRepositoryId
                    swappedPath
                    (Sha256Hash "guarded-file-dir-remote-directory")
                    (Blake3Hash "guarded-file-dir-remote-directory-blake3")
                    (List<DirectoryVersionId>())
                    (List<FileVersion>())
                    0L
            ...
            let remoteRoot =
                DirectoryVersion.CreateWithHashes
                    ...
                    currentRepositoryId
                    Constants.RootDirectoryPath
                    (Sha256Hash "guarded-file-dir-remote-root")
                    (Blake3Hash "guarded-file-dir-remote-root-blake3")
                    (List<DirectoryVersionId>([| remoteDirectoryId |]))
                    (List<FileVersion>())
                    0L
    ...
    /// Verifies that a failed authorized delete preserves its original error and stops later directory work.
    [<Test>]
    let ``current branch materialization file to directory type swap preserves delete failure`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            ...
            let remoteRootId = Guid.NewGuid()
            let remoteDirectoryId = Guid.NewGuid()
            let swappedPath = RelativePath "locked-file-to-directory"
            let swappedFilePath = Path.Combine(root, string swappedPath)

            File.WriteAllText(swappedFilePath, "locked tracked file")
            ...
            let previousRoot =
                LocalDirectoryVersion.CreateWithHashes
                    ...
                    currentRepositoryId
                    Constants.RootDirectoryPath
                    (Sha256Hash "locked-file-dir-local-root")
                    (Blake3Hash "locked-file-dir-local-root-blake3")
                    (List<DirectoryVersionId>())
                    (List<LocalFileVersion>([| previousFile |]))
                    previousFile.Size
                    DateTime.UtcNow
            ...
            let remoteDirectory =
                DirectoryVersion.CreateWithHashes
                    ...
                    currentRepositoryId
                    swappedPath
                    (Sha256Hash "locked-file-dir-remote-directory")
                    (Blake3Hash "locked-file-dir-remote-directory-blake3")
                    (List<DirectoryVersionId>())
                    (List<FileVersion>())
                    0L
            ...
            let remoteRoot =
                DirectoryVersion.CreateWithHashes
                    ...
                    currentRepositoryId
                    Constants.RootDirectoryPath
                    (Sha256Hash "locked-file-dir-remote-root")
                    (Blake3Hash "locked-file-dir-remote-root-blake3")
                    (List<DirectoryVersionId>([| remoteDirectoryId |]))
                    (List<FileVersion>())
                    0L
    ...
    [<Test>]
    let ``current branch materialization preflights all target guards before remote writes`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            Watch.clearGraceWatchResyncPendingForWatchTests ()
            ...
            let localRootId = Guid.NewGuid()
            let remoteRootId = Guid.NewGuid()
            let firstRelativePath = RelativePath "first-target.txt"
            let secondRelativePath = RelativePath "second-target.txt"
            let firstWorkingPath = Path.Combine(root, string firstRelativePath)
            let secondWorkingPath = Path.Combine(root, string secondRelativePath)
            let firstPreviousBytes = Encoding.UTF8.GetBytes("first previous clean content")
            let secondPreviousBytes = Encoding.UTF8.GetBytes("second previous clean content")
            let firstRemoteBytes = Encoding.UTF8.GetBytes("first remote materialized content")
            let secondRemoteBytes = Encoding.UTF8.GetBytes("second remote materialized content")
            let secondRacingBytes = Encoding.UTF8.GetBytes("second local edit before preflight finishes")

            File.WriteAllBytes(firstWorkingPath, firstPreviousBytes)
            File.WriteAllBytes(secondWorkingPath, secondPreviousBytes)

            let previousFiles =
                [|
                    (Services.createLocalFileVersion (FileInfo(firstWorkingPath)))
                        .GetAwaiter()
                        .GetResult()
                        .Value
                    (Services.createLocalFileVersion (FileInfo(secondWorkingPath)))
                        .GetAwaiter()
                        .GetResult()
                        .Value
                |]
            ...
            let previousRoot =
                LocalDirectoryVersion.CreateWithHashes
                    ...
                    currentRepositoryId
                    Constants.RootDirectoryPath
                    (Sha256Hash "multi-target-local-root")
                    (Blake3Hash "multi-target-local-root-blake3")
                    (List<DirectoryVersionId>())
                    (List<LocalFileVersion>(previousFiles))
                    (int64 (
                        firstPreviousBytes.Length
                        + secondPreviousBytes.Length
                    ))
                    DateTime.UtcNow
            ...
            let remoteFiles =
                [|
                    FileVersion.CreateWithHashes
                        firstRelativePath
                        (Sha256Hash "multi-target-first-remote")
                        (Blake3Hash "multi-target-first-remote-blake3")
                        String.Empty
                        false
                        (int64 firstRemoteBytes.Length)
                    FileVersion.CreateWithHashes
                        secondRelativePath
                        (Sha256Hash "multi-target-second-remote")
                        (Blake3Hash "multi-target-second-remote-blake3")
                        String.Empty
                        false
                        (int64 secondRemoteBytes.Length)
                |]

            for remoteFile, remoteBytes in
                Array.zip
                    remoteFiles
                    [|
                        firstRemoteBytes
            ...
            let remoteRoot =
                DirectoryVersion.CreateWithHashes
                    ...
                    currentRepositoryId
                    Constants.RootDirectoryPath
                    (Sha256Hash "multi-target-remote-root")
                    (Blake3Hash "multi-target-remote-root-blake3")
                    (List<DirectoryVersionId>())
                    (List<FileVersion>(remoteFiles))
                    (int64 (firstRemoteBytes.Length + secondRemoteBytes.Length))

            let remoteDto =
                { DirectoryVersionDto.Default with DirectoryVersion = remoteRoot; RecursiveSize = int64 (firstRemoteBytes.Length + secondRemoteBytes.Length) }
            ...
            let payload = currentBranchReferencePayload currentRepositoryId currentBranchId remoteRootId remoteRoot.Sha256Hash remoteRoot.Blake3Hash
            let secondGuardCalls = ref 0

            let result =
                (Services.updateWorkingDirectoryWithTargetGuard previousStatus updatedStatus [| remoteDto |] (generateCorrelationId ()) (fun mutation ->
                    if String.Equals(mutation.FullPath, secondWorkingPath, StringComparison.OrdinalIgnoreCase) then
                        incr secondGuardCalls

                        if secondGuardCalls.Value = 1 then
                            File.WriteAllBytes(secondWorkingPath, secondRacingBytes)
            ...
            File.ReadAllBytes(firstWorkingPath)
            |> should equal firstPreviousBytes

            File.ReadAllBytes(secondWorkingPath)
            |> should equal secondRacingBytes

            Watch.isGraceWatchResyncPendingForWatchTests ()
            |> should equal true)

    /// Verifies that recursive directory deletion verifies the full tree before removing children.
    [<Test>]
    let ``current branch materialization target guard verifies directory tree before deletion`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            ...
            let verifiedTargets = List<string>()
            let parentDirectoryVerificationCount = ref 0

            let result =
                (Services.updateWorkingDirectoryWithTargetGuard previousStatus updatedStatus [| remoteDto |] (generateCorrelationId ()) (fun mutation ->
                    task {
                        verifiedTargets.Add(Path.GetRelativePath(root, mutation.FullPath))

                        if
                            mutation.IsDirectory
                            && String.Equals(mutation.FullPath, deletedDirectoryPath, StringComparison.OrdinalIgnoreCase)
                        then
                            incr parentDirectoryVerificationCount

                            if parentDirectoryVerificationCount.Value = 1 then
                                return Ok()
                            else
                                return Error(GraceError.Create "test-controlled parent directory verification failure" (generateCorrelationId ()))
                        else
                            return Ok()
                    }))
                    .GetAwaiter()
            ...
            File.ReadAllText(deletedChildPath)
            |> should equal "old clean child"

            File.ReadAllText(nestedChildPath)
            |> should equal "old clean nested child"

            Directory.Exists(nestedDirectoryPath)
            |> should equal true

            Directory.Exists(deletedDirectoryPath)
            |> should equal true)

    /// Verifies that recursive directory deletion can remove the tree after every target is verified.
    [<Test>]
    let ``current branch materialization target guard deletes directory tree after verification`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
    ...
    /// Verifies that a child file is revalidated immediately before directory-tree deletion removes it.
    [<Test>]
    let ``current branch materialization revalidates directory child file at delete boundary`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            Watch.clearGraceWatchResyncPendingForWatchTests ()
            ...
            let deletedDirectoryId = Guid.NewGuid()
            let remoteRootId = Guid.NewGuid()
            let deletedDirectoryRelativePath = RelativePath "remote-deleted-racing-dir"
            let deletedDirectoryPath = Path.Combine(root, string deletedDirectoryRelativePath)
            let deletedChildPath = Path.Combine(deletedDirectoryPath, "old-child.txt")
            ...
            let previousDeletedDirectory =
                LocalDirectoryVersion.CreateWithHashes
                    ...
                    currentRepositoryId
                    deletedDirectoryRelativePath
                    (Sha256Hash "directory-delete-race-child-local")
                    (Blake3Hash "directory-delete-race-child-local-blake3")
                    (List<DirectoryVersionId>())
                    (List<LocalFileVersion>([| previousChild |]))
                    previousChild.Size
                    DateTime.UtcNow
            ...
            let previousRoot =
                LocalDirectoryVersion.CreateWithHashes
                    ...
                    currentRepositoryId
                    Constants.RootDirectoryPath
                    (Sha256Hash "directory-delete-race-root-local")
                    (Blake3Hash "directory-delete-race-root-local-blake3")
                    (List<DirectoryVersionId>([| deletedDirectoryId |]))
                    (List<LocalFileVersion>())
                    previousChild.Size
                    DateTime.UtcNow
            ...
            let remoteRoot =
                DirectoryVersion.CreateWithHashes
                    ...
                    currentRepositoryId
                    Constants.RootDirectoryPath
                    (Sha256Hash "directory-delete-race-root-remote")
                    (Blake3Hash "directory-delete-race-root-remote-blake3")
                    (List<DirectoryVersionId>())
                    (List<FileVersion>())
                    0L
            ...
            let payload = currentBranchReferencePayload currentRepositoryId currentBranchId remoteRootId remoteRoot.Sha256Hash remoteRoot.Blake3Hash
            let childGuardCalls = ref 0

            let result =
                (Services.updateWorkingDirectoryWithTargetGuard previousStatus updatedStatus [| remoteDto |] (generateCorrelationId ()) (fun mutation ->
                    if String.Equals(mutation.FullPath, deletedChildPath, StringComparison.OrdinalIgnoreCase) then
                        incr childGuardCalls

                        if childGuardCalls.Value = 3 then
                            File.WriteAllText(deletedChildPath, "racing local child edit")
            ...
            childGuardCalls.Value |> should equal 3

            File.ReadAllText(deletedChildPath)
            |> should equal "racing local child edit"
    ...
    /// Verifies that production materialized publication records one anonymous stale-root credit for one old root.
    [<Test>]
    let ``current branch materialization production publication records one anonymous stale-root credit`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            Watch.clearPendingWatchWorkForTests ()
            Services.setGraceWatchHasPendingWorkForStatus false
            ...
            let oldStatus =
                { GraceStatus.Default with
                    RootDirectoryId = oldRootId
                    RootDirectorySha256Hash = Sha256Hash "single-credit-old-root"
                    RootDirectoryBlake3Hash = Blake3Hash "single-credit-old-root-blake3"
                }

            let newStatus =
                { GraceStatus.Default with
                    RootDirectoryId = newRootId
                    RootDirectorySha256Hash = Sha256Hash "single-credit-new-root"
                    RootDirectoryBlake3Hash = Blake3Hash "single-credit-new-root-blake3"
                }

            Watch.recordPublishedCurrentBranchCleanRootForWatchTests oldStatus

            let cleanOldStatus =
                { liveWatchStatus oldRootId with
                    RootDirectorySha256Hash = oldStatus.RootDirectorySha256Hash
                    RootDirectoryBlake3Hash = oldStatus.RootDirectoryBlake3Hash
                    DirectoryIds = HashSet<DirectoryVersionId>([| oldRootId |])
                }

            let newPayload =
                currentBranchReferencePayload currentRepositoryId currentBranchId newRootId newStatus.RootDirectorySha256Hash newStatus.RootDirectoryBlake3Hash

            let materializingOperations, _, _, _, _ =
                recordingMaterializationOperations (ref (materializationInspection None cleanOldStatus)) oldStatus newStatus [| DirectoryVersionDto.Default |]

            let productionPublishingOperations =
                { materializingOperations with
                    PublishWatchStatusRecordsSupersededRoot = true
                    PublishWatchStatus =
                        fun authority status ->
                            task {
                                match! Watch.publishCurrentBranchMaterializedStatusForAuthorityForWatchTests authority status with
                                | Ok () -> ()
                                | Error error -> return raise (InvalidOperationException(error.Error))
                            }
                }

            (Watch.processCurrentBranchReferenceMaterializationForWatchTests productionPublishingOperations true [| newPayload |])
                .GetAwaiter()
                .GetResult()
            |> ignore

            let cleanNewStatus =
                { liveWatchStatus newRootId with
                    RootDirectorySha256Hash = newStatus.RootDirectorySha256Hash
                    RootDirectoryBlake3Hash = newStatus.RootDirectoryBlake3Hash
                    DirectoryIds = HashSet<DirectoryVersionId>([| newRootId |])
                }

            let oldAnonymousPayload =
                { currentBranchReferencePayload
                      currentRepositoryId
                      currentBranchId
                      oldRootId
                      oldStatus.RootDirectorySha256Hash
                      oldStatus.RootDirectoryBlake3Hash with
                    ReferenceId = ReferenceId.Empty
                }

            let skippedOperations, skippedFetchedRoots, skippedDownloadBatches, skippedAppliedPayloads, skippedPublishedStatuses =
                recordingMaterializationOperations (ref (materializationInspection None cleanNewStatus)) newStatus oldStatus [| DirectoryVersionDto.Default |]

            (Watch.processCurrentBranchReferenceMaterializationForWatchTests skippedOperations true [| oldAnonymousPayload |])
                .GetAwaiter()
                .GetResult()
            |> ignore

            skippedFetchedRoots.Count |> should equal 0
            skippedDownloadBatches.Count |> should equal 0
            skippedAppliedPayloads.Count |> should equal 0
            skippedPublishedStatuses.Count |> should equal 0

            let exhaustedOperations, exhaustedFetchedRoots, _, exhaustedAppliedPayloads, exhaustedPublishedStatuses =
                recordingMaterializationOperations (ref (materializationInspection None cleanNewStatus)) newStatus oldStatus [| DirectoryVersionDto.Default |]

            (Watch.processCurrentBranchReferenceMaterializationForWatchTests exhaustedOperations true [| oldAnonymousPayload |])
                .GetAwaiter()
                .GetResult()
            |> ignore
    ...
    /// Verifies that same-root clean status refreshes do not advance the publication generation.
    [<Test>]
    let ``current branch materialization same root publication does not advance generation`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            ...
            let cleanStatus =
                { GraceStatus.Default with
                    RootDirectoryId = Guid.NewGuid()
                    RootDirectorySha256Hash = Sha256Hash "same-root-generation"
                    RootDirectoryBlake3Hash = Blake3Hash "same-root-generation-blake3"
                }

            let before = Watch.currentPublishedCurrentBranchCleanRootGenerationForWatchTests ()

            Watch.recordPublishedCurrentBranchCleanRootForWatchTests cleanStatus

            let afterFirstPublication = Watch.currentPublishedCurrentBranchCleanRootGenerationForWatchTests ()

            afterFirstPublication
            |> should equal (before + 1L)

            Watch.recordPublishedCurrentBranchCleanRootForWatchTests cleanStatus

            Watch.currentPublishedCurrentBranchCleanRootGenerationForWatchTests ()
            |> should equal afterFirstPublication)
    ...
    let ``current branch materialization local clean publication records anonymous retired root`` () =
        withTempRepo (fun root ->
            ...
            let olderLocalStatus =
                { GraceStatus.Default with
                    RootDirectoryId = olderLocalRootId
                    RootDirectorySha256Hash = Sha256Hash "anonymous-retired-local"
                    RootDirectoryBlake3Hash = Blake3Hash "anonymous-retired-local-blake3"
                }

            let newerLocalStatus =
                { GraceStatus.Default with
                    RootDirectoryId = newerLocalRootId
                    RootDirectorySha256Hash = Sha256Hash "anonymous-retired-newer-local"
                    RootDirectoryBlake3Hash = Blake3Hash "anonymous-retired-newer-local-blake3"
                }

            let retiredAnonymousPayload =
                { currentBranchReferencePayload
                      currentRepositoryId
                      currentBranchId
                      olderLocalRootId
            ...
            let duplicateOperations, duplicateFetchedRoots, duplicateDownloadBatches, duplicateAppliedPayloads, duplicatePublishedStatuses =
                recordingMaterializationOperations
                    (ref (materializationInspection None cleanStatusAfterLocalSave))
                    newerLocalStatus
                    olderLocalStatus
            ...
            (Watch.processCurrentBranchReferenceMaterializationForWatchTests duplicateOperations true [| retiredAnonymousPayload |])
                .GetAwaiter()
                .GetResult()
            |> ignore
            ...
            let unrelatedAnonymousPayload =
                { retiredAnonymousPayload with
                    DirectoryId = Guid.NewGuid()
                    Sha256Hash = Sha256Hash "anonymous-retired-unrelated"
                    Blake3Hash = Blake3Hash "anonymous-retired-unrelated-blake3"
                }

            let unrelatedOperations, unrelatedFetchedRoots, unrelatedDownloadBatches, unrelatedAppliedPayloads, unrelatedPublishedStatuses =
                recordingMaterializationOperations
                    (ref (materializationInspection None cleanStatusAfterLocalSave))
                    newerLocalStatus
                    { olderLocalStatus with
                        RootDirectoryId = unrelatedAnonymousPayload.DirectoryId
                        RootDirectorySha256Hash = unrelatedAnonymousPayload.Sha256Hash
                        RootDirectoryBlake3Hash = unrelatedAnonymousPayload.Blake3Hash
                    }
                    [| DirectoryVersionDto.Default |]

            (Watch.processCurrentBranchReferenceMaterializationForWatchTests unrelatedOperations true [| unrelatedAnonymousPayload |])
                .GetAwaiter()
                .GetResult()
            |> ignore

            unrelatedFetchedRoots
            |> Seq.toArray
            |> should
                equal
                [|
                    unrelatedAnonymousPayload.DirectoryId
                |]

            unrelatedDownloadBatches.Count |> should equal 1

            unrelatedAppliedPayloads
            |> Seq.map (fun payload -> payload.DirectoryId)
            |> Seq.toArray
            |> should
                equal
                [|
            ...
            unrelatedPublishedStatuses.Count |> should equal 1)

    /// Verifies that a newer Reference records stale authority for an anonymous pending root it supersedes.
    [<Test>]
    let ``current branch materialization reference supersession records anonymous retired root`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            ...
            let olderStatus =
                { GraceStatus.Default with
                    RootDirectorySha256Hash = Sha256Hash "anonymous-reference-older-root"
                    RootDirectoryBlake3Hash = Blake3Hash "anonymous-reference-older-root-blake3"
                    RootDirectoryId = olderRootId
                }

            let newerStatus =
                { GraceStatus.Default with
                    RootDirectorySha256Hash = Sha256Hash "anonymous-reference-newer-root"
                    RootDirectoryBlake3Hash = Blake3Hash "anonymous-reference-newer-root-blake3"
                    RootDirectoryId = newerRootId
                }

            let olderAnonymousPayload =
                { currentBranchReferencePayload
                      currentRepositoryId
                      currentBranchId
                      olderRootId
                      olderStatus.RootDirectorySha256Hash
                      olderStatus.RootDirectoryBlake3Hash with
                    ReferenceId = ReferenceId.Empty
                }

            let newerReferencePayload =
                currentBranchReferencePayload
                    currentRepositoryId
                    currentBranchId
                    newerRootId
                    newerStatus.RootDirectorySha256Hash
                    newerStatus.RootDirectoryBlake3Hash

            Watch.rememberPendingCurrentBranchMaterializationForWatchTests
                olderAnonymousPayload
                Services.LatestCurrentBranchReferenceDecisionReason.RemoteMaterializationRequired

            let olderWatchStatus =
                { liveWatchStatus olderRootId with
                    RootDirectorySha256Hash = olderStatus.RootDirectorySha256Hash
                    RootDirectoryBlake3Hash = olderStatus.RootDirectoryBlake3Hash
                    DirectoryIds = HashSet<DirectoryVersionId>([| olderRootId |])
                }

            let inspection = ref (materializationInspection None olderWatchStatus)

            let newerOperations, newerFetchedRoots, _, newerAppliedPayloads, newerPublishedStatuses =
                recordingMaterializationOperations inspection olderStatus newerStatus [| DirectoryVersionDto.Default |]

            (Watch.processCurrentBranchReferenceMaterializationForWatchTests newerOperations true [| newerReferencePayload |])
                .GetAwaiter()
                .GetResult()
            |> ignore
            ...
            newerAppliedPayloads
            |> Seq.map (fun payload -> payload.ReferenceId)
            |> Seq.toArray
            |> should equal [| newerReferencePayload.ReferenceId |]

            newerPublishedStatuses.Count |> should equal 1
            ...
            let newerWatchStatus =
                { liveWatchStatus newerRootId with
                    RootDirectorySha256Hash = newerStatus.RootDirectorySha256Hash
                    RootDirectoryBlake3Hash = newerStatus.RootDirectoryBlake3Hash
                    DirectoryIds = HashSet<DirectoryVersionId>([| newerRootId |])
                }

            inspection.Value <- materializationInspection None newerWatchStatus

            let duplicateOperations, duplicateFetchedRoots, duplicateDownloadBatches, duplicateAppliedPayloads, duplicatePublishedStatuses =
                recordingMaterializationOperations inspection newerStatus olderStatus [| DirectoryVersionDto.Default |]

            (Watch.processCurrentBranchReferenceMaterializationForWatchTests duplicateOperations true [| olderAnonymousPayload |])
                .GetAwaiter()
                .GetResult()
            |> ignore
    ...
    let ``current branch materialization rejects delayed older root after tree replacement`` () =
        withTempRepo (fun root ->
            ...
            let olderStatus =
                { liveWatchStatus olderRootId with
                    RootDirectorySha256Hash = Sha256Hash "older-replaced-root"
                    RootDirectoryBlake3Hash = Blake3Hash "older-replaced-root-blake3"
                }

            let newerStatus =
                { liveWatchStatus newerRootId with
                    RootDirectorySha256Hash = Sha256Hash "newer-replacement-root"
                    RootDirectoryBlake3Hash = Blake3Hash "newer-replacement-root-blake3"
                    DirectoryIds = HashSet<DirectoryVersionId>([| newerRootId |])
                }
            ...
            let olderPayload =
                currentBranchReferencePayload
                    ...
                    olderRootId
                    olderStatus.RootDirectorySha256Hash
                    olderStatus.RootDirectoryBlake3Hash

            let inspection = ref (materializationInspection None olderStatus)

            Watch.rememberPendingCurrentBranchMaterializationForWatchTests
                olderPayload
                Services.LatestCurrentBranchReferenceDecisionReason.RemoteMaterializationRequired

            let firstOperations, firstFetchedRoots, _, firstAppliedPayloads, firstPublishedStatuses =
                recordingMaterializationOperations inspection previousStatus updatedStatus [| DirectoryVersionDto.Default |]

            (Watch.processCurrentBranchReferenceMaterializationForWatchTests firstOperations true [| newerPayload |])
                .GetAwaiter()
                .GetResult()
            |> ignore
            ...
            firstAppliedPayloads
            |> Seq.map (fun payload -> payload.ReferenceId)
            |> Seq.toArray
            |> should equal [| newerPayload.ReferenceId |]

            firstPublishedStatuses.Count |> should equal 1

            inspection.Value <- materializationInspection None newerStatus

            let secondOperations, secondFetchedRoots, secondDownloadBatches, secondAppliedPayloads, secondPublishedStatuses =
                recordingMaterializationOperations inspection updatedStatus previousStatus [| DirectoryVersionDto.Default |]

            let staleDecision =
                (Watch.processCurrentBranchReferenceMaterializationForWatchTests secondOperations true [| olderPayload |])
                    .GetAwaiter()
                    .GetResult()
            ...
            secondFetchedRoots.Count |> should equal 0
            secondDownloadBatches.Count |> should equal 0
    ...
    /// Verifies that materialization marker deletion does not enter the branch-transition completion path.
    [<Test>]
    let ``current branch materialization marker deletion does not complete branch transition`` () =
        withTempRepo (fun root ->
            let markerFileName = Services.updateInProgressFileName ()
            let changedRelativePath = "materialized-remote-file.txt"
            let changedFilePath = Path.Combine(root, changedRelativePath)
            let transitionCompletionCalls = ref 0

            try
                let sidecarObservedBeforeDelete = ref false
                Watch.setBranchTransitionCompletionAfterRetireProbeForWatchTests (fun () -> incr transitionCompletionCalls)

                Watch.setCurrentBranchMaterializationMarkerDeleteForWatchTests (fun marker ->
                    sidecarObservedBeforeDelete.Value <- File.Exists(marker + ".completed")
                    File.Delete(marker))

                Watch.setReadGraceStatusFileForWatchTests (fun () -> Task.FromResult(graceStatusTracking [| changedRelativePath |] Array.empty<string>))

                let result =
                    (Watch.runCurrentBranchMaterializationWithMarkerForWatchTests
                        markerFileName
                        "`grace watch` current-branch materialization is in progress."
                        (fun () ->
                            File.WriteAllText(changedFilePath, "remote materialized content")
                            Task.FromResult<Result<unit, GraceError>>(Ok())))
                        .GetAwaiter()
                        .GetResult()
                ...
                sidecarObservedBeforeDelete.Value
                |> should equal true

                let completedUtc =
                    File.ReadAllText(markerFileName + ".completed")
                    |> DateTime.Parse

                File.SetLastWriteTimeUtc(changedFilePath, completedUtc.AddMilliseconds(-1.0))

                Watch.OnGraceUpdateInProgressDeleted(deletedEvent markerFileName)
                Watch.OnChanged(changedEvent changedFilePath)

                transitionCompletionCalls.Value |> should equal 0
    ...
    /// Verifies that current-branch materialization refuses an update marker already owned by another Grace operation.
    [<Test>]
    let ``current branch materialization refuses existing update marker`` () =
        withTempRepo (fun _ ->
            let markerFileName = Services.updateInProgressFileName ()
            let markerText = "`grace watch` current-branch materialization is in progress."
            let existingMarkerText = "`grace switch` is in progress."
            let operationCalled = ref false

            Directory.CreateDirectory(Path.GetDirectoryName(markerFileName))
            |> ignore

            File.WriteAllText(markerFileName, existingMarkerText)
            ...
            result.IsError |> should equal true
            operationCalled.Value |> should equal false

            File.ReadAllText(markerFileName)
            |> should equal existingMarkerText
    ...
    /// Verifies that failed abort cleanup returns the marker cleanup failure instead of preserving incremental confidence.
    [<Test>]
    let ``current branch materialization abort cleanup failure fails closed`` () =
        withTempRepo (fun _ ->
            let markerFileName = Services.updateInProgressFileName ()
            let markerText = "`grace watch` current-branch materialization is in progress."
            let operationError = GraceError.Create "simulated materialization abort" (generateCorrelationId ())
            ...
            try
                ...
                let result =
                    (Watch.runCurrentBranchMaterializationWithMarkerForWatchTests markerFileName markerText (fun () ->
                        Task.FromResult<Result<unit, GraceError>>(Error operationError)))
                        .GetAwaiter()
                        .GetResult()

                match result with
                | Ok () -> Assert.Fail("Expected marker cleanup failure to replace the operation abort result.")
                | Error error ->
                    error.Error.Contains("could not prove its owned Grace update marker was deleted", StringComparison.Ordinal)
                    |> should equal true

                    error.Error.Contains(operationError.Error, StringComparison.Ordinal)
                    |> should equal false
        ...
        withTempRepo (fun root ->
            ...
            let payload =
                currentBranchReferencePayload
                    ...
                    currentBranchId
                    (Guid.NewGuid())
                    (Sha256Hash "marker-close-failure-root")
                    (Blake3Hash "marker-close-failure-root-blake3")
            ...
            try
                ...
                let result =
                    (Watch.runDurablyAppliedCurrentBranchMaterializationWithMarkerAndCloseVerificationForWatchTests
                        payload
                        markerFileName
                        markerText
                        (fun () -> Task.FromResult<Result<unit, GraceError>>(Ok()))
    ...
    /// Verifies that losing the owned marker lease before cleanup fails closed.
    [<Test>]
    let ``current branch materialization marker disappearance fails closed`` () =
        withTempRepo (fun _ ->
            let markerFileName = Services.updateInProgressFileName ()
            let markerText = "`grace watch` current-branch materialization is in progress."
    ...
    [<Test>]
    let ``current branch materialization failure does not write completed sidecar or suppress callback`` () =
        withTempRepo (fun root ->
            Watch.clearPendingWatchWorkForTests ()

            let markerFileName = Services.updateInProgressFileName ()
            let changedRelativePath = "partial-materialization.txt"
            let changedFilePath = Path.Combine(root, changedRelativePath)
            ...
            (fun () ->
                (Watch.runCurrentBranchMaterializationWithMarkerForWatchTests
                    ...
                    "`grace watch` current-branch materialization is in progress."
                    (fun () ->
                        File.WriteAllText(changedFilePath, "partial remote materialization")
                        Task.FromException<Result<unit, GraceError>>(InvalidOperationException("simulated partial materialization failure"))))
                    .GetAwaiter()
                    .GetResult()
                |> ignore)
            |> should throw typeof<InvalidOperationException>

            File.Exists(markerFileName) |> should equal false
    ...
    /// Verifies that marker-window local edits are detected before completion evidence can suppress callbacks.
    [<Test>]
    let ``current branch materialization marker window scan catches local edit before success`` () =
        withTempRepo (fun root ->
            Watch.clearPendingWatchWorkForTests ()

            let remoteStatus = graceStatusTracking Array.empty<string> Array.empty<string>
            let localEditPath = Path.Combine(root, "marker-window-local-edit.txt")

            File.WriteAllText(localEditPath, "local edit while materialization marker is present")

            let result =
                (Watch.verifyCurrentBranchMaterializationMarkerWindowCleanForWatchTests (Guid.NewGuid()) remoteStatus (generateCorrelationId ()))
                    .GetAwaiter()
                    .GetResult()
    ...
    /// Verifies that the final marker-window scan runs after marker close and cannot hide post-close user edits.
    [<Test>]
    let ``current branch materialization post close scan catches local edit without suppressing callback`` () =
        withTempRepo (fun root ->
            Watch.clearPendingWatchWorkForTests ()
            ...
            let markerText = "`grace watch` current-branch materialization is in progress."
            let remoteStatus = graceStatusTracking Array.empty<string> Array.empty<string>
            let localEditPath = Path.Combine(root, "post-close-local-edit.txt")
            let referenceId = Guid.NewGuid()

            let result =
                (Watch.runCurrentBranchMaterializationWithMarkerAndCloseVerificationForWatchTests
                    markerFileName
                    markerText
                    (fun () -> Task.FromResult<Result<unit, GraceError>>(Ok()))
                    (fun completedUtc ->
                        task {
                            ...
                            File.WriteAllText(localEditPath, "local edit after materialization marker close")
                            File.SetLastWriteTimeUtc(localEditPath, completedUtc.AddMilliseconds(1.0))

                            return! Watch.verifyCurrentBranchMaterializationMarkerWindowCleanForWatchTests referenceId remoteStatus (generateCorrelationId ())
                        }))
                    .GetAwaiter()
                    .GetResult()
            ...
            Watch.OnChanged(changedEvent localEditPath)

            let pending = Watch.pendingWatchWorkSnapshotForTests ()

            pending.FilesToProcess
            |> should equal [| localEditPath |])

    /// Verifies that marker-time content verification catches edits not reflected in Watch IPC state.
    [<Test>]
    let ``current branch materialization pre write content scan blocks unobserved local edit`` () =
        withTempRepo (fun root ->
            Watch.clearPendingWatchWorkForTests ()

            let previousStatus = graceStatusTracking Array.empty<string> Array.empty<string>
            File.WriteAllText(Path.Combine(root, "unobserved-before-write.txt"), "local edit before remote overwrite")

            let result =
                (Watch.verifyCurrentBranchMaterializationPreWriteContentCleanForWatchTests (Guid.NewGuid()) previousStatus (generateCorrelationId ()))
                    .GetAwaiter()
                    .GetResult()
    ...
    let ``current branch materialization rechecks pending work before remote write`` () =
        withTempRepo (fun root ->
            ...
            let remoteRootId = Guid.NewGuid()
            let cleanStatus = liveWatchStatus localRootId
            let pendingStatus = { cleanStatus with HasPendingWatchWork = true; IsWorkingTreeClean = false }
            ...
            let payload =
                currentBranchReferencePayload
                    ...
                    currentBranchId
                    remoteRootId
                    (Sha256Hash "pre-write-pending-root")
                    (Blake3Hash "pre-write-pending-root-blake3")
                    ...
                    InspectWatchStatus =
                        fun () ->
                            ...
                            if inspectionReadCount.Value = 1 then
                                ...
                                Task.FromResult(materializationInspection None pendingStatus)
                    ReadGraceStatus = fun () -> Task.FromResult(previousStatus)
                    GetDirectoryVersionsRecursive =
                        fun _ _ -> Task.FromResult<Result<DirectoryVersionDto array, GraceError>>(Ok [| DirectoryVersionDto.Default |])
                    DownloadDirectoryFiles = fun _ _ -> Task.FromResult<Result<unit, GraceError>>(Ok())
                    ApplyRemoteDirectory =
            ...
            inspectionReadCount.Value |> should equal 2
            appliedPayloads.Count |> should equal 0
            publishedStatuses.Count |> should equal 0
    ...
    /// Verifies that an active remote payload is retired when a local save advances the root during remote download.
    [<Test>]
    let ``current branch materialization local root advance during download consumes remote payload`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            ...
            let newerLocalStatus =
                { liveWatchStatus newerLocalRootId with
                    RootDirectorySha256Hash = Sha256Hash "newer-local-save-root"
                    RootDirectoryBlake3Hash = Blake3Hash "newer-local-save-root-blake3"
                    DirectoryIds = HashSet<DirectoryVersionId>([| newerLocalRootId |])
                }
            ...
            let payload =
                currentBranchReferencePayload
                    ...
                    currentBranchId
                    remoteRootId
                    (Sha256Hash "download-race-remote-root")
                    (Blake3Hash "download-race-remote-root-blake3")
            ...
            let operations, fetchedRoots, downloadBatches, appliedPayloads, publishedStatuses =
                recordingMaterializationOperations inspection previousStatus GraceStatus.Default [| DirectoryVersionDto.Default |]
                    ...
                    ReadGraceStatus =
                        fun () ->
                            ...
                            if readCount.Value = 1 then
                                ...
                                inspection.Value <- materializationInspection None newerLocalStatus
                                Task.FromResult(newerLocalGraceStatus)
                }
            ...
            let retryOperations, retryFetchedRoots, retryDownloadBatches, retryAppliedPayloads, retryPublishedStatuses =
                recordingMaterializationOperations
                    (ref (materializationInspection None newerLocalStatus))
                    newerLocalGraceStatus
                    GraceStatus.Default
                    [| DirectoryVersionDto.Default |]
    ...
    /// Verifies that marker-time local edit evidence retires the queued remote payload before resync.
    [<Test>]
    let ``current branch materialization pre write local edit consumes remote payload`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            Watch.clearGraceWatchResyncPendingForWatchTests ()
            ...
            let payload =
                currentBranchReferencePayload
                    ...
                    currentBranchId
                    remoteRootId
                    (Sha256Hash "pre-write-local-edit-remote")
                    (Blake3Hash "pre-write-local-edit-remote-blake3")

            Watch.rememberPendingCurrentBranchMaterializationForWatchTests
                payload
                Services.LatestCurrentBranchReferenceDecisionReason.RemoteMaterializationRequired

            File.WriteAllText(Path.Combine(root, "pre-write-local-edit.txt"), "local edit before remote overwrite")

            let result =
                (Watch.verifyCurrentBranchMaterializationPreWriteContentCleanAndRetireStalePayloadForWatchTests
                    payload
                    previousStatus
                    (generateCorrelationId ()))
                    .GetAwaiter()
            ...
            let localAfterResync =
                { liveWatchStatus localRootId with
                    RootDirectorySha256Hash = Sha256Hash "after-pre-write-local-edit"
                    RootDirectoryBlake3Hash = Blake3Hash "after-pre-write-local-edit-blake3"
                    DirectoryIds = HashSet<DirectoryVersionId>([| localRootId |])
                }
    ...
    /// Verifies that a clean marker-time scan does not falsely retire a queued remote payload.
    [<Test>]
    let ``current branch materialization clean pre write scan preserves remote payload`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            Watch.clearGraceWatchResyncPendingForWatchTests ()
            ...
            let payload =
                currentBranchReferencePayload
                    ...
                    currentBranchId
                    remoteRootId
                    (Sha256Hash "pre-write-clean-remote")
                    (Blake3Hash "pre-write-clean-remote-blake3")
    ...
    /// Verifies that marker-time scanner failures preserve the remote payload for retry.
    [<Test>]
    let ``current branch materialization pre write scanner failure preserves remote payload`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            Watch.clearGraceWatchResyncPendingForWatchTests ()
            ...
            let payload =
                currentBranchReferencePayload
                    ...
                    currentBranchId
                    remoteRootId
                    (Sha256Hash "pre-write-scan-failure-remote")
                    (Blake3Hash "pre-write-scan-failure-remote-blake3")
            ...
            let current = Current()
            let originalRoot = current.RootDirectory
            current.RootDirectory <- root + string (char 0)
            ...
            let result =
                try
                    ...
                    current.RootDirectory <- originalRoot
    ...
    [<Test>]
    let ``stale materialization tag with newer live marker does not skip branch transition completion`` () =
        withTempRepo (fun _ ->
            let markerFileName = Services.updateInProgressFileName ()
            let transitionCompletionCalls = ref 0
            ...
            try
                ...
                materializationResult.IsOk |> should equal true

                File.WriteAllText(markerFileName, "`grace switch` is in progress.")
                Watch.OnGraceUpdateInProgressCreated(createdEvent markerFileName)
                Watch.OnGraceUpdateInProgressDeleted(deletedEvent markerFileName)

                let switchCompletedUtc = DateTime.UtcNow
                File.WriteAllText(markerFileName + ".completed", switchCompletedUtc.ToString("O"))
                File.Delete(markerFileName)
                Watch.OnGraceUpdateInProgressDeleted(deletedEvent markerFileName)

                transitionCompletionCalls.Value |> should equal 1
            finally
                Watch.resetBranchTransitionCompletionAfterRetireProbeForWatchTests ())

    /// Verifies that rootless current-branch References are discarded instead of retained for retry.
    [<Test>]
    let ``current branch materialization drops rootless reference without pending retry`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            ...
            let payload =
                { currentBranchReferencePayload currentRepositoryId currentBranchId DirectoryVersionId.Empty (Sha256Hash "") (Blake3Hash "") with
                    ReferenceId = Guid.NewGuid()
                }
            ...
            let operations, fetchedRoots, downloadBatches, appliedPayloads, publishedStatuses =
                recordingMaterializationOperations inspection GraceStatus.Default GraceStatus.Default Array.empty<DirectoryVersionDto>
    ...
    /// Verifies that a rootless concrete Reference does not retire newer rootful pending materialization work.
    [<Test>]
    let ``current branch materialization rootless concrete reference preserves newer pending materialization`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            ...
            let olderPayload =
                currentBranchReferencePayload
                    ...
                    currentBranchId
                    olderRemoteRootId
                    (Sha256Hash "older-pending-root")
                    (Blake3Hash "older-pending-root-blake3")
            ...
            let rootlessOperations, _, _, _, _ =
                recordingMaterializationOperations
                    (ref (materializationInspection None cleanStatus))
                    GraceStatus.Default
                    GraceStatus.Default
            ...
            (Watch.processCurrentBranchReferenceMaterializationForWatchTests rootlessOperations true [| rootlessPayload |])
                .GetAwaiter()
                .GetResult()
            |> ignore
            ...
            pendingStatus.LatestDirectoryId
            |> should equal (Some olderPayload.DirectoryId))

    /// Verifies that a partial concrete Reference does not retire newer rootful pending materialization work.
    [<Test>]
    let ``current branch materialization partial concrete reference preserves newer pending materialization`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
    ...
    [<Test>]
    let ``current branch materialization retry catches failure and keeps pending reference`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            ...
            let payload =
                currentBranchReferencePayload
                    ...
                    currentBranchId
                    remoteRootId
                    (Sha256Hash "retry-exception-root")
                    (Blake3Hash "retry-exception-root-blake3")

            let inspection = ref (materializationInspection None dirtyStatus)

            let operations, _, _, _, _ = recordingMaterializationOperations inspection GraceStatus.Default GraceStatus.Default Array.empty<DirectoryVersionDto>
            ...
            let throwingRetryOperations = { operations with InspectWatchStatus = fun () -> raise (InvalidOperationException("simulated retry status failure")) }

            let retryDecision =
                (Watch.retryPendingCurrentBranchMaterializationForWatchTests throwingRetryOperations)
                    .GetAwaiter()
                    .GetResult()
    ...
    /// Verifies that a retry cannot clear a newer deferred root queued while the retry owns the materialization gate.
    [<Test>]
    let ``current branch materialization retry preserves newer pending tail`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()

            let currentRepositoryId, currentBranchId = configureCurrentWatchIdentity root "current-repo" "current-branch"
            let localRootId = Guid.NewGuid()
            let retryRootId = Guid.NewGuid()
            let newerRootId = Guid.NewGuid()
            let cleanStatus = liveWatchStatus localRootId
            ...
            let retryPayload =
                currentBranchReferencePayload
                    ...
                    currentBranchId
                    retryRootId
                    (Sha256Hash "retry-tail-root")
                    (Blake3Hash "retry-tail-root-blake3")
            ...
            let newerPayload =
                currentBranchReferencePayload
                    ...
                    currentBranchId
                    newerRootId
                    (Sha256Hash "newer-tail-root")
                    (Blake3Hash "newer-tail-root-blake3")
            ...
            let updatedStatus =
                { previousStatus with
                    RootDirectoryId = retryRootId
                    RootDirectorySha256Hash = retryPayload.Sha256Hash
                    RootDirectoryBlake3Hash = retryPayload.Blake3Hash
                }
            ...
            let enqueueingOperations =
                { operations with
                    ApplyRemoteDirectory =
                        fun payload authority graceStatus dtos correlationId ->
                            Watch.rememberPendingCurrentBranchMaterializationForWatchTests
                                newerPayload
                                Services.LatestCurrentBranchReferenceDecisionReason.RemoteMaterializationRequired

                            operations.ApplyRemoteDirectory payload authority graceStatus dtos correlationId
                }

            let decision =
                (Watch.retryPendingCurrentBranchMaterializationForWatchTests enqueueingOperations)
                    .GetAwaiter()
                    .GetResult()
            ...
            |> Seq.map (fun payload -> payload.ReferenceId)
            |> Seq.toArray
            |> should equal [| retryPayload.ReferenceId |]
            ...
            pendingStatus.LatestReferenceId
            |> should equal (Some newerPayload.ReferenceId))

    /// Verifies that stale-reference discard cannot append an older root after a newer deferred tail.
    [<Test>]
    let ``current branch materialization stale discard preserves newer pending tail`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            ...
            let olderStatus =
                { liveWatchStatus olderRootId with
                    RootDirectorySha256Hash = Sha256Hash "stale-tail-older-root"
                    RootDirectoryBlake3Hash = Blake3Hash "stale-tail-older-root-blake3"
                }

            let currentStatus =
                { liveWatchStatus currentRootId with
                    RootDirectorySha256Hash = Sha256Hash "stale-tail-current-root"
                    RootDirectoryBlake3Hash = Blake3Hash "stale-tail-current-root-blake3"
                    DirectoryIds = HashSet<DirectoryVersionId>([| currentRootId |])
                }
            ...
            let newerPendingPayload =
                currentBranchReferencePayload
                    ...
                    currentBranchId
                    newerPendingRootId
                    (Sha256Hash "stale-tail-newer-pending")
                    (Blake3Hash "stale-tail-newer-pending-blake3")
            ...
            let firstOperations, _, _, _, _ = recordingMaterializationOperations inspection previousStatus materializedStatus [| DirectoryVersionDto.Default |]

            (Watch.processCurrentBranchReferenceMaterializationForWatchTests firstOperations true [| currentPayload |])
                .GetAwaiter()
                .GetResult()
            |> ignore
            ...
            inspection.Value <- materializationInspection None currentStatus

            let staleOperations, staleFetchedRoots, staleDownloadBatches, staleAppliedPayloads, stalePublishedStatuses =
                recordingMaterializationOperations inspection materializedStatus previousStatus [| DirectoryVersionDto.Default |]

            (Watch.processCurrentBranchReferenceMaterializationForWatchTests staleOperations true [| stalePayload |])
                .GetAwaiter()
                .GetResult()
            |> ignore

            staleFetchedRoots.Count |> should equal 0
            staleDownloadBatches.Count |> should equal 0
            staleAppliedPayloads.Count |> should equal 0
            stalePublishedStatuses.Count |> should equal 0
            ...
            pendingStatus.LatestReferenceId
            |> should equal (Some newerPendingPayload.ReferenceId))

    /// Verifies that a delayed same-root notification does not clear newer deferred remote materialization state.
    [<Test>]
    let ``current branch materialization no-op preserves newer pending reference`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()

            let currentRepositoryId, currentBranchId = configureCurrentWatchIdentity root "current-repo" "current-branch"
            let localRootId = Guid.NewGuid()
            let deferredRootId = Guid.NewGuid()
            let cleanStatus = liveWatchStatus localRootId
            ...
            let noOpPayload =
                currentBranchReferencePayload
                    ...
                    currentBranchId
                    localRootId
                    cleanStatus.RootDirectorySha256Hash
                    cleanStatus.RootDirectoryBlake3Hash
            ...
            let deferredPayload =
                currentBranchReferencePayload
                    ...
                    currentBranchId
                    deferredRootId
                    (Sha256Hash "newer-deferred-root")
                    (Blake3Hash "newer-deferred-root-blake3")
            ...
            let decision =
                (Watch.processCurrentBranchReferenceMaterializationForWatchTests operations true [| noOpPayload |])
                    .GetAwaiter()
                    .GetResult()
            ...
            pendingStatus.LatestReferenceId
            |> should equal (Some deferredPayload.ReferenceId))

    /// Verifies that retry collapse records skipped pending roots before clearing the applied tail.
    [<Test>]
    let ``current branch materialization retry records skipped roots before tail clear`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()

            let currentRepositoryId, currentBranchId = configureCurrentWatchIdentity root "current-repo" "current-branch"
            let localRootId = Guid.NewGuid()
            let rootAId = Guid.NewGuid()
            let rootBId = Guid.NewGuid()
            let rootCId = Guid.NewGuid()
            let cleanStatus = liveWatchStatus localRootId

            let payloadA = currentBranchReferencePayload currentRepositoryId currentBranchId rootAId (Sha256Hash "skipped-a") (Blake3Hash "skipped-a-blake3")

            let payloadB = currentBranchReferencePayload currentRepositoryId currentBranchId rootBId (Sha256Hash "skipped-b") (Blake3Hash "skipped-b-blake3")

            let payloadC = currentBranchReferencePayload currentRepositoryId currentBranchId rootCId (Sha256Hash "applied-c") (Blake3Hash "applied-c-blake3")
            ...
            let materializedStatus =
                { previousStatus with RootDirectoryId = rootCId; RootDirectorySha256Hash = payloadC.Sha256Hash; RootDirectoryBlake3Hash = payloadC.Blake3Hash }

            for payload in [ payloadA; payloadB; payloadC ] do
                Watch.rememberPendingCurrentBranchMaterializationForWatchTests
                    payload
                    Services.LatestCurrentBranchReferenceDecisionReason.RemoteMaterializationRequired
            ...
            |> Seq.map (fun payload -> payload.ReferenceId)
            |> Seq.toArray
            |> should equal [| payloadC.ReferenceId |]
            ...
            let afterCStatus =
                { liveWatchStatus rootCId with
                    RootDirectorySha256Hash = payloadC.Sha256Hash
                    RootDirectoryBlake3Hash = payloadC.Blake3Hash
                    DirectoryIds = HashSet<DirectoryVersionId>([| rootCId |])
                }

            let delayedBOperations, delayedBFetchedRoots, delayedBDownloadBatches, delayedBAppliedPayloads, delayedBPublishedStatuses =
                recordingMaterializationOperations
                    (ref (materializationInspection None afterCStatus))
                    materializedStatus
                    previousStatus
                    [| DirectoryVersionDto.Default |]

            (Watch.processCurrentBranchReferenceMaterializationForWatchTests delayedBOperations true [| payloadB |])
                .GetAwaiter()
                .GetResult()
            |> ignore

            delayedBFetchedRoots.Count |> should equal 0
            delayedBDownloadBatches.Count |> should equal 0
        ...
        withTempRepo (fun root ->
            ...
            let newerTailPayload =
                currentBranchReferencePayload
                    ...
                    currentBranchId
                    newerTailRootId
                    (Sha256Hash "duplicate-applied-newer-tail")
                    (Blake3Hash "duplicate-applied-newer-tail-blake3")
            ...
            Watch.consumeDurablyAppliedCurrentBranchMaterializationBeforeResyncForWatchTests appliedPayload
            ...
            pendingStatus.LatestReferenceId
            |> should equal (Some newerTailPayload.ReferenceId)

            let localAfterResync =
                { liveWatchStatus (Guid.NewGuid()) with
                    RootDirectorySha256Hash = Sha256Hash "after-duplicate-consumption"
                    RootDirectoryBlake3Hash = Blake3Hash "after-duplicate-consumption-blake3"
            ...
            (Watch.processCurrentBranchReferenceMaterializationForWatchTests retryOperations true [| appliedPayload |])
                .GetAwaiter()
                .GetResult()
            |> ignore
    ...
    /// Verifies that a post-status-write failure consumes the applied payload before retry can replay it.
    [<Test>]
    let ``current branch materialization status boundary failure consumes applied payload`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            Watch.clearGraceWatchResyncPendingForWatchTests ()
            ...
            let payload =
                currentBranchReferencePayload
                    ...
                    currentBranchId
                    appliedRootId
                    (Sha256Hash "status-boundary-applied-root")
                    (Blake3Hash "status-boundary-applied-root-blake3")

            Watch.rememberPendingCurrentBranchMaterializationForWatchTests
                payload
                Services.LatestCurrentBranchReferenceDecisionReason.RemoteMaterializationRequired

            let markerFileName = Path.Combine(root, ".grace", "status-boundary-materialization.marker")
            let markerText = "`grace watch` status boundary failure test."
            ...
            let result =
                (Watch.runDurablyAppliedCurrentBranchMaterializationWithMarkerAndCloseVerificationForWatchTests
                    ...
                    (fun () ->
                        task {
                            Watch.consumeDurablyAppliedCurrentBranchMaterializationBeforeResyncForWatchTests payload

                            return Error(GraceError.Create "simulated object-cache failure after durable status write" (generateCorrelationId ()))
                        })
                    (fun _ -> Task.FromResult(Ok())))
                    .GetAwaiter()
                    .GetResult()
            ...
            let localAfterResync =
                { liveWatchStatus laterLocalRootId with
                    RootDirectorySha256Hash = Sha256Hash "later-local-after-status-boundary"
                    RootDirectoryBlake3Hash = Blake3Hash "later-local-after-status-boundary-blake3"
        ...
        withTempRepo (fun root ->
            ...
            let payload =
                currentBranchReferencePayload
                    ...
                    currentBranchId
                    remoteRootId
                    (Sha256Hash "publish-failure-root")
                    (Blake3Hash "publish-failure-root-blake3")
            ...
            let operations, _, _, appliedPayloads, _ =
                recordingMaterializationOperations
                    (ref (materializationInspection None cleanStatus))
                    previousStatus
                    materializedStatus
            ...
            let failingPublishOperations =
                { operations with PublishWatchStatus = fun _ _ -> Task.FromException<unit>(InvalidOperationException("simulated IPC publication failure")) }

            (Watch.processCurrentBranchReferenceMaterializationForWatchTests failingPublishOperations true [| payload |])
                .GetAwaiter()
                .GetResult()
            |> ignore
    ...
    /// Verifies that same-root branch identity drift at the write boundary blocks remote materialization.
    [<Test>]
    let ``current branch materialization write boundary rejects changed branch identity`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            Watch.clearGraceWatchResyncPendingForWatchTests ()
            ...
            let switchedStatus = { cleanStatus with BranchId = Guid.NewGuid(); BranchName = BranchName "switched-branch" }
            ...
            let payload =
                currentBranchReferencePayload
                    ...
                    currentBranchId
                    remoteRootId
                    (Sha256Hash "write-boundary-branch-remote")
                    (Blake3Hash "write-boundary-branch-remote-blake3")
                ...
                { operations with
                    ReadGraceStatus =
                        fun () ->
                            ...
                            if readCount.Value = 2 then
                                inspection.Value <- materializationInspection None switchedStatus
    ...
    let ``current branch materialization ipc verification failure returns publication error`` () =
        withTempRepo (fun root ->
            ...
            let result =
                (Watch.publishCurrentBranchMaterializedStatusWithWriterForWatchTests materializedStatus (fun _ _ -> Task.FromResult(())))
                    .GetAwaiter()
                    .GetResult()
    ...
    [<Test>]
    let ``current branch materialization marker window failure blocks ipc publication`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            Watch.clearPendingWatchWorkForTests ()
            ...
            let verifyMarkerWindow () =
                task {
                    ordering.Add("verify")
                    File.WriteAllText(localEditPath, "local edit before IPC publication")
            ...
            ordering.ToArray() |> should equal [| "verify" |]
    ...
    [<Test>]
    let ``current branch materialization consumes applied root before marker window resync`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            ...
            let payload =
                currentBranchReferencePayload
                    ...
                    currentBranchId
                    appliedRootId
                    (Sha256Hash "applied-before-resync")
                    (Blake3Hash "applied-before-resync-blake3")
            ...
            let localAfterResync =
                { liveWatchStatus laterLocalRootId with
                    RootDirectorySha256Hash = Sha256Hash "later-local-root"
                    RootDirectoryBlake3Hash = Blake3Hash "later-local-root-blake3"
                    DirectoryIds = HashSet<DirectoryVersionId>([| laterLocalRootId |])
                }
        ...
        withTempRepo (fun root ->
            ...
            let delayedStartupPayload =
                { currentBranchReferencePayload
                      ...
                      startupStatus.RootDirectoryBlake3Hash with
                    ReferenceId = ReferenceId.Empty
                }
    ...
    /// Verifies that retiring one queued anonymous payload grants exactly one later stale-root rejection credit.
    [<Test>]
    let ``current branch materialization boundary path retires one queued anonymous payload with one rejection credit`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            ...
            let oldStatus =
                { GraceStatus.Default with
                    RootDirectoryId = oldRootId
                    RootDirectorySha256Hash = Sha256Hash "anonymous-single-old-root"
                    RootDirectoryBlake3Hash = Blake3Hash "anonymous-single-old-root-blake3"
                }

            let newerStatus =
                { GraceStatus.Default with
                    RootDirectoryId = newerRootId
                    RootDirectorySha256Hash = Sha256Hash "anonymous-single-new-root"
                    RootDirectoryBlake3Hash = Blake3Hash "anonymous-single-new-root-blake3"
                }
            ...
            Watch.consumeDurablyAppliedCurrentBranchMaterializationBeforeResyncForWatchTests anonymousPayload

            let skippedOperations, skippedFetchedRoots, skippedDownloads, skippedAppliedPayloads, skippedPublishedStatuses =
                recordingMaterializationOperations
                    (ref (materializationInspection None liveNewerStatus))
                    newerStatus
                    oldStatus
                    [| DirectoryVersionDto.Default |]

            (Watch.processCurrentBranchReferenceMaterializationForWatchTests skippedOperations true [| anonymousPayload |])
                .GetAwaiter()
                .GetResult()
            |> ignore

            skippedFetchedRoots.Count |> should equal 0
            skippedDownloads.Count |> should equal 0
            skippedAppliedPayloads.Count |> should equal 0
            skippedPublishedStatuses.Count |> should equal 0

            let materializingOperations, materializingFetchedRoots, materializingDownloads, materializingAppliedPayloads, materializingPublishedStatuses =
                recordingMaterializationOperations
                    (ref (materializationInspection None liveNewerStatus))
                    newerStatus
                    oldStatus
            ...
            (Watch.processCurrentBranchReferenceMaterializationForWatchTests materializingOperations true [| anonymousPayload |])
                .GetAwaiter()
                .GetResult()
            |> ignore

            materializingFetchedRoots.ToArray()
            |> should equal [| oldRootId |]

            materializingDownloads.Count |> should equal 1

            materializingAppliedPayloads
            |> Seq.map (fun payload -> payload.ReferenceId)
            |> Seq.toArray
            |> should equal [| ReferenceId.Empty |]

            materializingPublishedStatuses.Count
            |> should equal 1)

    /// Verifies that dropping an anonymous stale live payload while applying deferred work spends its stale-root credit.
    [<Test>]
    let ``current branch materialization dropped anonymous stale live payload consumes credit`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            ...
            let oldStatus =
                { GraceStatus.Default with
                    RootDirectoryId = oldRootId
                    RootDirectorySha256Hash = Sha256Hash "anonymous-dropped-old-root"
                    RootDirectoryBlake3Hash = Blake3Hash "anonymous-dropped-old-root-blake3"
                }

            let newerStatus =
                { GraceStatus.Default with
                    RootDirectoryId = newerRootId
                    RootDirectorySha256Hash = Sha256Hash "anonymous-dropped-new-root"
                    RootDirectoryBlake3Hash = Blake3Hash "anonymous-dropped-new-root-blake3"
                }

            let liveCurrentStatus =
                { liveWatchStatus currentRootId with
                    RootDirectorySha256Hash = Sha256Hash "anonymous-dropped-current-root"
                    RootDirectoryBlake3Hash = Blake3Hash "anonymous-dropped-current-root-blake3"
                    DirectoryIds = HashSet<DirectoryVersionId>([| currentRootId |])
                }
            ...
            let currentStatus =
                { GraceStatus.Default with
                    RootDirectoryId = currentRootId
                    RootDirectorySha256Hash = liveCurrentStatus.RootDirectorySha256Hash
                    RootDirectoryBlake3Hash = liveCurrentStatus.RootDirectoryBlake3Hash
                }
            ...
            Watch.rememberPendingCurrentBranchMaterializationForWatchTests
                staleAnonymousPayload
                Services.LatestCurrentBranchReferenceDecisionReason.RemoteMaterializationRequired

            Watch.consumeDurablyAppliedCurrentBranchMaterializationBeforeResyncForWatchTests staleAnonymousPayload

            Watch.rememberPendingCurrentBranchMaterializationForWatchTests
                deferredNewerPayload
                Services.LatestCurrentBranchReferenceDecisionReason.LocalStatusRequiresResync

            let deferredOperations, deferredFetchedRoots, _, deferredAppliedPayloads, _ =
                recordingMaterializationOperations
                    (ref (materializationInspection None liveCurrentStatus))
                    currentStatus
                    newerStatus
                    [| DirectoryVersionDto.Default |]

            (Watch.processCurrentBranchReferenceMaterializationForWatchTests deferredOperations true [| staleAnonymousPayload |])
                .GetAwaiter()
                .GetResult()
            |> ignore

            deferredFetchedRoots.ToArray()
            |> should equal [| newerRootId |]

            deferredAppliedPayloads
            |> Seq.map (fun payload -> payload.ReferenceId)
            |> Seq.toArray
            |> should equal [| deferredNewerPayload.ReferenceId |]

            let materializingOperations, materializingFetchedRoots, _, materializingAppliedPayloads, _ =
                recordingMaterializationOperations
                    (ref (materializationInspection None liveNewerStatus))
                    newerStatus
                    oldStatus
            ...
            (Watch.processCurrentBranchReferenceMaterializationForWatchTests materializingOperations true [| staleAnonymousPayload |])
                .GetAwaiter()
                .GetResult()
            |> ignore
    ...
    /// Verifies that anonymous stale-root history does not reject a concrete new Reference by root alone.
    [<Test>]
    let ``current branch materialization allows concrete reference returning to anonymous stale root`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            ...
            let oldStatus =
                { GraceStatus.Default with
                    RootDirectoryId = oldRootId
                    RootDirectorySha256Hash = Sha256Hash "anonymous-old-root"
                    RootDirectoryBlake3Hash = Blake3Hash "anonymous-old-root-blake3"
                }

            let newerStatus =
                { GraceStatus.Default with
                    RootDirectoryId = newerRootId
                    RootDirectorySha256Hash = Sha256Hash "anonymous-new-root"
                    RootDirectoryBlake3Hash = Blake3Hash "anonymous-new-root-blake3"
                }

            Watch.recordPublishedCurrentBranchCleanRootForWatchTests oldStatus
            Watch.recordPublishedCurrentBranchCleanRootForWatchTests newerStatus
            ...
            let returningPayload =
                currentBranchReferencePayload currentRepositoryId currentBranchId oldRootId oldStatus.RootDirectorySha256Hash oldStatus.RootDirectoryBlake3Hash
            ...
            (Watch.processCurrentBranchReferenceMaterializationForWatchTests operations true [| returningPayload |])
                .GetAwaiter()
                .GetResult()
            |> ignore
            ...
            |> Seq.map (fun payload -> payload.ReferenceId)
            |> Seq.toArray
            |> should equal [| returningPayload.ReferenceId |]

            publishedStatuses.Count |> should equal 1)

    /// Verifies that a new Reference may legitimately return to a previously superseded root.
    [<Test>]
    let ``current branch materialization accepts newer reference returning to superseded root`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            ...
            let oldRootId = Guid.NewGuid()
            let newerRootId = Guid.NewGuid()
            let currentStatus = { liveWatchStatus localRootId with DirectoryIds = HashSet<DirectoryVersionId>([| localRootId |]) }

            let skippedOldPayload =
                currentBranchReferencePayload currentRepositoryId currentBranchId oldRootId (Sha256Hash "old-root") (Blake3Hash "old-root-blake3")

            let newerPayload =
                currentBranchReferencePayload currentRepositoryId currentBranchId newerRootId (Sha256Hash "new-root") (Blake3Hash "new-root-blake3")

            let legitimateReturnPayload =
                { skippedOldPayload with
                    ReferenceId = Guid.NewGuid()
                    Sha256Hash = Sha256Hash "old-root-return"
                    Blake3Hash = Blake3Hash "old-root-return-blake3"
                }

            let currentGraceStatus =
                { GraceStatus.Default with
                    RootDirectoryId = localRootId
                    RootDirectorySha256Hash = currentStatus.RootDirectorySha256Hash
                    RootDirectoryBlake3Hash = currentStatus.RootDirectoryBlake3Hash
            ...
            let returnGraceStatus =
                { GraceStatus.Default with
                    RootDirectoryId = oldRootId
                    RootDirectorySha256Hash = legitimateReturnPayload.Sha256Hash
                    RootDirectoryBlake3Hash = legitimateReturnPayload.Blake3Hash
                }
            ...
            let operations, _, _, _, _ =
                recordingMaterializationOperations
                    (ref (materializationInspection None currentStatus))
                    currentGraceStatus
                    newerGraceStatus
                    [| DirectoryVersionDto.Default |]
            ...
            let returnOperations, fetchedRoots, _, appliedPayloads, _ =
                recordingMaterializationOperations
                    (ref (materializationInspection None newerStatus))
                    newerGraceStatus
                    returnGraceStatus
                    [| DirectoryVersionDto.Default |]

            (Watch.processCurrentBranchReferenceMaterializationForWatchTests returnOperations true [| legitimateReturnPayload |])
                .GetAwaiter()
                .GetResult()
            |> ignore
            ...
            |> should
                equal
                [|
                    legitimateReturnPayload.ReferenceId
                |])

    /// Verifies that consuming two queued anonymous duplicates preserves two bounded later stale-root rejections.
    [<Test>]
    let ``current branch materialization keeps one stale-root rejection credit per consumed anonymous duplicate`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            ...
            let oldStatus =
                { GraceStatus.Default with
                    RootDirectoryId = oldRootId
                    RootDirectorySha256Hash = Sha256Hash "anonymous-duplicate-old-root"
                    RootDirectoryBlake3Hash = Blake3Hash "anonymous-duplicate-old-root-blake3"
                }

            let newerStatus =
                { GraceStatus.Default with
                    RootDirectoryId = newerRootId
                    RootDirectorySha256Hash = Sha256Hash "anonymous-duplicate-new-root"
                    RootDirectoryBlake3Hash = Blake3Hash "anonymous-duplicate-new-root-blake3"
                }
            ...
            let firstSkippedOperations, firstSkippedFetchedRoots, firstSkippedDownloads, firstSkippedAppliedPayloads, firstSkippedPublishedStatuses =
                recordingMaterializationOperations
                    (ref (materializationInspection None liveNewerStatus))
                    newerStatus
                    oldStatus
            ...
            (Watch.processCurrentBranchReferenceMaterializationForWatchTests firstSkippedOperations true [| anonymousPayload |])
                .GetAwaiter()
                .GetResult()
            |> ignore

            firstSkippedFetchedRoots.Count |> should equal 0
            firstSkippedDownloads.Count |> should equal 0

            firstSkippedAppliedPayloads.Count
            |> should equal 0

            firstSkippedPublishedStatuses.Count
            |> should equal 0

            let secondSkippedOperations, secondSkippedFetchedRoots, secondSkippedDownloads, secondSkippedAppliedPayloads, secondSkippedPublishedStatuses =
                recordingMaterializationOperations
                    (ref (materializationInspection None liveNewerStatus))
                    newerStatus
                    oldStatus
            ...
            (Watch.processCurrentBranchReferenceMaterializationForWatchTests secondSkippedOperations true [| anonymousPayload |])
                .GetAwaiter()
                .GetResult()
            |> ignore

            secondSkippedFetchedRoots.Count |> should equal 0
            secondSkippedDownloads.Count |> should equal 0

            secondSkippedAppliedPayloads.Count
            |> should equal 0

            secondSkippedPublishedStatuses.Count
            |> should equal 0
    ...
    /// Verifies that deferred remote materialization state is scoped to the active branch.
    [<Test>]
    let ``current branch materialization clears deferred reference after branch identity changes`` () =
        withTempRepo (fun root ->
            Watch.clearPendingCurrentBranchMaterializationsForWatchTests ()
            ...
            let payload =
                currentBranchReferencePayload
                    ...
                    currentBranchId
                    (Guid.NewGuid())
                    (Sha256Hash "branch-scoped-root")
                    (Blake3Hash "branch-scoped-root-blake3")
    ...
    /// Verifies that materialized status publication refreshes both IPC and the in-process directory-id cache.
    [<Test>]
    let ``current branch materialization publishes fresh directory ids through watch ipc gate`` () =
        withTempRepo (fun root ->
            configureCurrentWatchIdentity root "current-repo" "current-branch"
            |> ignore

            let staleStatus = graceStatusTracking [| "stale.txt" |] Array.empty<string>
            ...
            let expectedDirectoryIds =
                materializedStatus.Index.Keys
                |> Seq.map id
                |> Set.ofSeq

            Watch.updateGraceStatusDirectoryIds staleStatus
            Services.setGraceWatchHasPendingWorkForStatus false

            let result =
                (Watch.publishCurrentBranchMaterializedStatusForWatchTests materializedStatus)
                    .GetAwaiter()
                    .GetResult()

            result.IsOk |> should equal true

            Watch.graceStatusDirectoryIdsForWatchTests ()
            |> Set.ofSeq
            |> should equal expectedDirectoryIds

            readWatchStatusJsonDirectoryIds ()
            |> should equal expectedDirectoryIds

            readWatchStatusJsonStringProperty "RootDirectoryId"
            |> should equal $"{materializedStatus.RootDirectoryId}")

    /// Verifies that materialized IPC publication updates DirectoryIds before the serialized snapshot write.
    [<Test>]
    let ``current branch materialization serializes directory id cache before ipc write`` () =
        withTempRepo (fun root ->
            configureCurrentWatchIdentity root "current-repo" "current-branch"
            |> ignore
            ...
            let writerObservedDirectoryIds = ref Set.empty<DirectoryVersionId>

            Watch.updateGraceStatusDirectoryIds staleStatus
            Services.setGraceWatchHasPendingWorkForStatus false

            let writeIpc status directoryIds =
                writerObservedDirectoryIds.Value <-
                    Watch.graceStatusDirectoryIdsForWatchTests ()
                    |> Set.ofSeq
