...
type BranchDtoHashTests() =
    ...
    [<Test>]
    member _.ReferenceProducingCommandsCarryStableIdentityAndBothRootHashes() =
        let commands =
            [
        ...
        for command in commands do
            match command with
            | BranchCommand.Assign (commandReferenceId, directoryId, sha, blake3, text)
            | BranchCommand.Promote (commandReferenceId, directoryId, sha, blake3, text)
            | BranchCommand.Commit (commandReferenceId, directoryId, sha, blake3, text)
            | BranchCommand.Checkpoint (commandReferenceId, directoryId, sha, blake3, text)
            | BranchCommand.Save (commandReferenceId, directoryId, sha, blake3, text)
            | BranchCommand.Tag (commandReferenceId, directoryId, sha, blake3, text)
            | BranchCommand.CreateExternal (commandReferenceId, directoryId, sha, blake3, text) ->
    ...
    /// Verifies stable Reference identity is present on the serialized public request wire shapes.
    [<Test>]
    member _.ReferenceProducerParametersSerializeStableIdentityOnThePublicWire() =
        let producerParameters: obj array =
            [|
                CreateBranchParameters(ReferenceId = referenceId)
                AssignParameters(ReferenceId = referenceId)
                RebaseParameters(ReferenceId = referenceId, BasedOn = Guid.NewGuid())
                CreateReferenceParameters(ReferenceId = referenceId)
                CommitReferenceParameters(ReferenceId = referenceId)
            |]

        for parameters in producerParameters do
            use json = JsonDocument.Parse(Utilities.serialize parameters)
            let mutable serializedReferenceId = Unchecked.defaultof<JsonElement>

            Assert.That(
                json.RootElement.TryGetProperty(nameof ReferenceId, &serializedReferenceId),
                Is.True,
                $"Expected {parameters.GetType().Name} to expose ReferenceId on its serialized request shape."
            )

            Assert.That(serializedReferenceId.GetGuid(), Is.EqualTo(referenceId))
