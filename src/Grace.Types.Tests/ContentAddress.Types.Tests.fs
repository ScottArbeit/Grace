...
/// Contains tests covering content address types behavior.
[<Parallelizable(ParallelScope.All)>]
type ContentAddressTypesTests() =
    /// Exercises run task coverage for the types content Address contract.
    let runTask (work: System.Threading.Tasks.Task<'T>) = work.GetAwaiter().GetResult()

    /// Verifies that blake3 known vectors pass.
    [<Test>]
    ...
    /// Verifies that file hash known vectors are byte only.
    [<Test>]
    ...
    /// Verifies that combined file hashes match known vectors.
    [<Test>]
    ...
    /// Verifies that file hashes ignore relative path.
    [<Test>]
    ...
    /// Verifies that same payload keeps version sha256 separate from cas blake3 address.
    [<Test>]
    ...
    /// Verifies that address validation requires lowercase sixty four hex characters.
    [<Test>]
    ...
    /// Verifies that chunk address from bytes uses lowercase blake3 hex.
    [<Test>]
    ...
    /// Verifies that content block preimage and address are stable and path independent.
    [<Test>]
    ...
    /// Verifies that manifest preimage and address are stable and path independent.
    [<Test>]
