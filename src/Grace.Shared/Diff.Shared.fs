...
module Diff =
    ...
    let processDiffModel (diffLines: List<DiffPiece>) (includeImaginary: bool) =
        ...
        for i = 0 to diffLines.Count - 1 do
            ...
            if diffLine.Type = ChangeType.Unchanged && (i < diffLines.Count - 1) && (diffLines[i + 1].Type = ChangeType.Unchanged) then
            ...
            elif diffLine.Type = ChangeType.Deleted || diffLine.Type = ChangeType.Inserted || diffLine.Type = ChangeType.Modified 
                    || (diffLine.Type = ChangeType.Imaginary && includeImaginary) then
    ...
    let diffTwoFiles (fileStream1: Stream) (fileStream2: Stream) =
        task {
            ...
            let inlineDiffPaneModel = inlineDiff.BuildDiffModel(fileContents1, fileContents2, ignoreWhitespace = true, ignoreCase = false, chunker = lineChunker)
