...
type OpenApiRouteCoverageTests() =
    ...
    member _.RouteClassificationRegistryCoversImplementedManifestAndOpenApiSurfaces() =
        ...
        let implementedRoutesAbsentFromOpenApi =
            startupRoutes
            |> Set.filter (fun route -> not (openApiPaths.Contains(openApiComparablePath route.Path)))
        ...
        let publicRoutesAlsoClassifiedNonPublic =
            startupRoutes
            |> Set.filter (fun route -> openApiPaths.Contains(openApiComparablePath route.Path))
    ...
    member _.OpenApiStalePathsAreExplicitlyClassified() =
        let implementedPaths =
            parseStartupRoutes ()
            |> Seq.map (fun route -> openApiComparablePath route.Path)
            |> Set.ofSeq
