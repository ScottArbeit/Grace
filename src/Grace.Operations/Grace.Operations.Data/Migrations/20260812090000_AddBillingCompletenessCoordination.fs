...
(
    ...
    CONSTRAINT CK_ops_UsageFactRejection_SuppliedGuids CHECK
    (
        RejectionId <> '00000000-0000-0000-0000-000000000000'
        AND (UsageFactId IS NULL OR UsageFactId <> '00000000-0000-0000-0000-000000000000')
        AND (OwnerId IS NULL OR OwnerId <> '00000000-0000-0000-0000-000000000000')
        AND (OrganizationId IS NULL OR OrganizationId <> '00000000-0000-0000-0000-000000000000')
        AND (RepositoryId IS NULL OR RepositoryId <> '00000000-0000-0000-0000-000000000000')
    ),
    CONSTRAINT CK_ops_UsageFactRejection_CompleteScopeRequiresFact CHECK
    (
        NOT (OwnerId IS NOT NULL AND OrganizationId IS NOT NULL AND RepositoryId IS NOT NULL AND MonthStartUtc IS NOT NULL)
        OR (UsageFactId IS NOT NULL AND UsageFactId <> '00000000-0000-0000-0000-000000000000')
    ),
    CONSTRAINT CK_ops_UsageFactRejection_MonthStartUtc CHECK
    (
        MonthStartUtc IS NULL
        OR MonthStartUtc = DATETIME2FROMPARTS(YEAR(MonthStartUtc), MONTH(MonthStartUtc), 1, 0, 0, 0, 0, 7)
    ),
    CONSTRAINT CK_ops_UsageFactRejection_Resolution CHECK
    (
        (IsActive = 1 AND ResolvedAtUtc IS NULL)
        OR (IsActive = 0 AND ResolvedAtUtc IS NOT NULL)
    ),
    CONSTRAINT CK_ops_UsageFactRejection_Reason CHECK (LEN(LTRIM(RTRIM(Reason))) > 0)
);
...
"""
    ...
    override _.Down(migrationBuilder: MigrationBuilder) =
        migrationBuilder.DropTable("UsageFactRejection", "ops")
        |> ignore

    /// Captures the complete Operations model represented by this migration as immutable literal metadata.
