...
/// Signals that zero-fact coverage remains deliberately outside this direct nonempty close slice.
type private ZeroFactCoveragePendingException() =
    inherit Exception("ZeroFactCoveragePending")
                ...
                if state = BillingPeriodState.Closed then
                    ...
                    if now < threshold then
                        ...
                        | None ->
                            ...
                            if List.isEmpty pricedFacts then raise (ZeroFactCoveragePendingException())
