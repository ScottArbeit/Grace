    ...
    member _.ScopedRejectionAndOnlineAcceptanceSerializeInBothCommitOrders() =
        withDatabaseAsync (fun connectionString ->
            task {
                ...
                Assert.That(blocked, Is.EqualTo(Complete))
            })
    ...
    member _.AcceptedFactsWinOverStaleRejectionAndDefaultPartialEvidenceHasNoMonth() =
        withDatabaseAsync (fun connectionString ->
            task {
                ...
                let! accepted = store.StoreUsageFactAsync(usageFact, payloadFor usageFact, CancellationToken.None)
                Assert.That(accepted.IsOk, Is.True)
                let! stale = store.RecordUsageFactRejectionAsync(scopedRejection usageFactId scope, CancellationToken.None)

                match stale with
                | Ok None -> ()
                | Ok (Some rejection) -> Assert.Fail($"Accepted usage must suppress stale blocker {rejection.RejectionId}.")
                ...
                let partial =
                    {
                        ...
                        Reason = "default timestamp is partial evidence"
                        IsActive = true
                    }

                let! recorded = store.RecordUsageFactRejectionAsync(partial, CancellationToken.None)

                match recorded with
                | Ok None -> ()
                | Ok (Some rejection) -> Assert.Fail($"Partial evidence must not acquire scope blocker {rejection.RejectionId}.")
                | Error errors -> Assert.Fail(String.Join("; ", errors))
