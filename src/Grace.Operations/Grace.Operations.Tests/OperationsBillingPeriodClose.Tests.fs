...
module private BillingCloseSchema =
    ...
    /// Names every table introduced by the close and late-work migrations with its SQL schema.
    let private tables =
    ...
    /// Separates SQL syntax from quote-delimited data while retaining each literal byte, including doubled quotes.
    let private tokenizeSql (value: string) =
        let tokens = ResizeArray<bool * string>()
        let syntax = Text.StringBuilder()

        let flushSyntax () =
            if syntax.Length > 0 then
                tokens.Add(false, syntax.ToString())
                syntax.Clear() |> ignore
        ...
        while index < value.Length do
            if value[index] <> '\'' then
                syntax.Append(value[index]) |> ignore
                index <- index + 1
            else
                flushSyntax ()
                ...
                tokens.Add(true, literal.ToString())

        flushSyntax ()
        tokens |> Seq.toList

    /// Normalizes only the finite syntax renderings emitted for this literal migration.
    let private normalizeSyntax (value: string) =
    ...
    /// Normalizes only token-safe syntax for checks and defaults; literal contents remain exactly distinguishable.
    ...
    let normalizeBillingPeriodDiagnosticDefinition (value: string) =
        let modelStateInBranch =
            tokenizeSql value
            |> List.exists (fun (isLiteral, token) ->
                not isLiteral
                && Regex.IsMatch(token, "\\[state\\]\\s+in\\s+\\(0\\s*,\\s*1\\)", RegexOptions.IgnoreCase))
    ...
    /// Normalizes only the named diagnostic check's documented SQL Server parenthesis rendering.
    let private normalizeCheckDefinition name definition =
        if name = "CK_ops_BillingPeriod_Diagnostic" then
            normalizeBillingPeriodDiagnosticDefinition definition
        else
    ...
    /// Extracts exact model metadata, resolving inferred textual collations only when a database default is supplied.
    let fromModelWithDatabaseDefault (databaseDefaultCollation: string) (model: IModel) =
        let entities =
                    ...
                    let columnEntry =
                        String.Join(
                            ...
                            [|
                                ...
                                if property.ClrType = typeof<string> then
                                    if String.IsNullOrWhiteSpace(property.GetCollation()) then
        ...
        let checks =
            ...
            |> Seq.collect (fun entity ->
                ...
                |> Seq.map (fun check -> $"{table}|{check.Name}|{normalizeCheckDefinition check.Name check.Sql}"))
            |> Set.ofSeq
    ...
    /// Extracts model metadata with an explicit marker for an unresolved textual database default.
    let fromModel (model: IModel) = fromModelWithDatabaseDefault "<database-default>" model
    ...
    /// Reads the actual disposable database default used by inferred textual model columns.
    let private databaseDefaultCollationAsync connectionString =
        task {
            use connection = new SqlConnection(connectionString)
            do! connection.OpenAsync CancellationToken.None
            use command = connection.CreateCommand()
...
SELECT CONCAT(s.name COLLATE DATABASE_DEFAULT,'.',t.name COLLATE DATABASE_DEFAULT,'|',c.name COLLATE DATABASE_DEFAULT,'|',
    ...
    '|',COALESCE(c.collation_name COLLATE DATABASE_DEFAULT,'-'))
FROM sys.tables t
...
"""
            ...
            let! databaseDefaultCollation = databaseDefaultCollationAsync connectionString
            ...
            return
                {
                    ...
                    Checks =
                        checks
                        |> Set.map (fun value -> let fields = value.Split('|', 3) in $"{fields[0]}|{fields[1]}|{normalizeCheckDefinition fields[1] fields[2]}")
                    Defaults =
                        defaults
...
"""
    ...
    [<Test>]
    member _.DiagnosticRenderingAndQuotedLiteralSafetySurviveNormalization() =
        let modelRendered =
        ...
        let serverRendered =
            "([RetryDiagnostic] IS NULL AND [RetryDiagnosticAtUtc] IS NULL OR ([State]=(1) OR [State]=(0)) AND len(ltrim(rtrim([RetryDiagnostic])))>(0) AND [RetryDiagnosticAtUtc] IS NOT NULL)"
        ...
        Assert.That(
            ...
            Does.Contain("'it''s exact'"),
            "Doubled quote escaping is data and must survive syntax normalization."
        )
    ...
    member _.RuntimeTargetSnapshotAndLiveSqlPhysicalSchemasAgreeExactly() =
        withDatabaseAsync (fun connectionString ->
            task {
                ...
                let! live, databaseDefaultCollation = BillingCloseSchema.fromLiveSqlAsync connectionString
                ...
                do!
                    executeAsync
                        connectionString
                        "ALTER TABLE ops.Charge DROP CONSTRAINT CK_ops_Charge_Currency; ALTER TABLE ops.Charge ADD CONSTRAINT CK_ops_Charge_Currency CHECK (LEN([CurrencyCode]) = 3 AND [CurrencyCode] = UPPER([CurrencyCode]) AND [CurrencyCode] NOT LIKE '%[^a-z]%');"

                let! liveWithLowercaseCurrency, _ = BillingCloseSchema.fromLiveSqlAsync connectionString

                let lowercaseCurrencyDrift =
                    BillingCloseSchema.compare runtime liveWithLowercaseCurrency false
                    |> String.concat Environment.NewLine

                Assert.That(lowercaseCurrencyDrift, Does.Contain("checks missing: ops.Charge|CK_ops_Charge_Currency"))
                Assert.That(lowercaseCurrencyDrift, Does.Contain("'%[^A-Z]%'"))
                Assert.That(lowercaseCurrencyDrift, Does.Contain("checks unexpected: ops.Charge|CK_ops_Charge_Currency"))
                Assert.That(lowercaseCurrencyDrift, Does.Contain("'%[^a-z]%'"))

                do!
                    executeAsync
                        connectionString
                        "ALTER TABLE ops.Charge DROP CONSTRAINT CK_ops_Charge_Provenance; ALTER TABLE ops.Charge ALTER COLUMN UnitName nvarchar(64) COLLATE Latin1_General_100_BIN2 NOT NULL; ALTER TABLE ops.Charge ADD CONSTRAINT CK_ops_Charge_Provenance CHECK ([PeriodFromUtc] < [PeriodToUtc] AND [PeriodFromUtc] <= [EffectiveFromUtc] AND [EffectiveFromUtc] < [EffectiveToUtc] AND [EffectiveToUtc] <= [PeriodToUtc] AND [FactKind] >= 0 AND [BillableUsageKind] >= 0 AND [BillableUsageKindMappingId] <> '00000000-0000-0000-0000-000000000000' AND [PricingAssignmentId] <> '00000000-0000-0000-0000-000000000000' AND [PricingPlanId] <> '00000000-0000-0000-0000-000000000000' AND [PricingRateId] <> '00000000-0000-0000-0000-000000000000' AND LEN(LTRIM(RTRIM([UnitName]))) > 0);"

                let! liveWithExplicitUnitNameCollation, _ = BillingCloseSchema.fromLiveSqlAsync connectionString

                let collationDrift =
                    BillingCloseSchema.compare runtime liveWithExplicitUnitNameCollation false
                    |> String.concat Environment.NewLine

                Assert.That(collationDrift, Does.Contain("columns missing: ops.Charge|UnitName"))
                Assert.That(collationDrift, Does.Contain("columns unexpected: ops.Charge|UnitName"))
                Assert.That(collationDrift, Does.Contain("Latin1_General_100_BIN2"))
                ...
                let! liveWithUnexpectedObject, _ = BillingCloseSchema.fromLiveSqlAsync connectionString
                ...
                let! liveWithMovedCharge, _ = BillingCloseSchema.fromLiveSqlAsync connectionString
