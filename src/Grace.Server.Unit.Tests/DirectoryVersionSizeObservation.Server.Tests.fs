...
open Grace.Server.DirectoryVersionSizeObservation
...
type DirectoryVersionSizeObservationTests() =
    ...
    member _.``handler reports unavailable configuration without observation or rollback promise``(capture: bool, invalid: bool, status: int) =
        task {
            ...
            Assert.That(response, Does.Not.Contain "DeclaredLogicalBytes")
    ...
    member _.``new capture checks scope around source and returns accepted winner``() =
        task {
            ...
            let winner = { reading with DeclaredLogicalBytes = 1L; EnumerationFinishedAt = reading.EnumerationStartedAt }
