...
open Grace.Server.ArtifactSizeObservation
...
type ArtifactSizeObservationTests() =
    ...
    member _.``handler reports unavailable configuration without observation or rollback promise``(capture: bool, invalid: bool, status: int) =
        task {
            ...
            Assert.That(response, Does.Not.Contain "DeclaredArtifactBytes")
    ...
    member _.``new capture checks scope around source and returns accepted winner``(winnerBytes: int64) =
        task {
            ...
            let winner =
                { reading with
                    DeclaredArtifactBytes = winnerBytes
