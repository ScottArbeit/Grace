...
module CacheArtifactStoreTestSupport =
    ...
    /// Returns the required restart outcome for one injected effect boundary.
    let expectedRestart effectName momentName =
        match effectName, momentName with
        | "final-file-publication", "after"
        | "complete-state-transaction", _
        | "terminal-success-publication", _ -> "Hit"
        | _ -> "Absent"
...
type CacheArtifactStoreTests() =
    ...
    member _.``success publishes verified bytes before the exact Complete state is visible``() =
        ...
        try
            ...
            | Hit _ -> ()
            | outcome -> Assert.Fail($"Same-tuple retry was not idempotent: {outcome}.")
        finally
            CacheArtifactStoreTestSupport.closeSession databasePath store

    /// Proves all sixteen effect interruptions classify to the exact accepted durable restart result.
    [<TestCaseSource(nameof CacheArtifactStoreTests.FailureCases)>]
    member _.``each injected boundary has the required restart residue``(effectName: string, momentName: string) =
        ...
        try
            ...
            try
                let expected = CacheArtifactStoreTestSupport.expectedRestart effectName momentName

                match expected, CacheArtifactStore.inspect restartedArtifacts tuple with
                | "Absent", Absent
                | "Hit", Hit _ -> ()
                | _, outcome -> Assert.Fail($"Restart classification was {outcome}, expected {expected} for {effectName}/{momentName}.")
    ...
    /// Confirms a mismatched byte stream cannot publish final bytes or Complete state and a later correct retry succeeds.
    [<Test>]
    member _.``integrity mismatch leaves no complete residue and permits a correct retry``() =
        let databasePath, _, store, artifacts = CacheArtifactStoreTestSupport.createSession ()
        let tuple = CacheArtifactStoreTestSupport.createTuple "artifact://grace/root/zip/integrity" CacheArtifactStoreTestSupport.payload
        let mismatched = Array.append CacheArtifactStoreTestSupport.payload [| 0uy |]

        try
            use invalidInput = CacheArtifactStoreTestSupport.source mismatched

            Assert.Throws<InvalidOperationException>(
                Action (fun () ->
                    CacheArtifactStore.commit artifacts tuple invalidInput
                    |> ignore)
            )
            |> ignore
            ...
            Assert.That(File.Exists(CacheArtifactStore.finalPathForTest artifacts tuple), Is.False)

            use validInput = CacheArtifactStoreTestSupport.source CacheArtifactStoreTestSupport.payload

            CacheArtifactStore.commit artifacts tuple validInput
            |> CacheArtifactStoreTestSupport.requireOutcome Filled
        finally
            CacheArtifactStoreTestSupport.closeSession databasePath store
    ...
    member _.``digest size root kind and canonical identity controls preserve completed bytes``() =
        ...
        try
            ...
            for name, candidate, expectedKind in controls do
                ...
                match expectedKind, CacheArtifactStore.commit artifacts candidate input with
                | ShouldConflict, Conflict _
                | ShouldReject, Rejected _
                | ShouldFill, Filled -> ()
                | _, outcome -> Assert.Fail($"{name} control returned {outcome}.")
    ...
    member _.``complete file disagreement requires local reset and preserves the durable record``() =
        ...
        try
            ...
            File.Delete(CacheArtifactStore.finalPathForTest artifacts tuple)
